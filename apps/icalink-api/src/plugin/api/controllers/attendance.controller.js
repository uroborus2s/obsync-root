 ...
 * 获取学生签到记录接口
 */
const getStudentAttendanceRecord = async (request, reply) => {
    try {
        const { id } = request.params;
        const { type = 'student' } = request.query;
        // 从DI容器中获取JWT payload
        const payload = request.diScope.resolve('userPayload');
        // 验证参数
        if (!id) {
        ...
        const courseScheduleRepo = request.diScope.resolve('courseScheduleRepo');
        // 获取考勤记录基本信息
        ...
        // 获取课程详细信息（教室和教师信息）
        const courseSchedules = await courseScheduleRepo.findByKkh(attendanceRecord.kkh);
        const courseInfo = courseSchedules.find((schedule) => schedule.rq === attendanceRecord.rq &&
            schedule.jc === parseInt(attendanceRecord.jc_s.split('-')[0] || '1')) || courseSchedules[0]; // 如果找不到精确匹配，使用第一条记录
        // 根据type参数选择相应的服务
        let userInfo;
        let userType;
        ...
        if (type === 'teacher') {
            // 创建教师服务实例
            const createTeacher = request.diScope.resolve('createTeacher');
            ...
            // 创建学生服务实例
            const createStudent = request.diScope.resolve('createStudent');
            ...
            try {
                ...
                if (studentData.xh) {
                    studentAttendance =
                        await studentAttendanceRepo.getStudentAttendanceByRecordAndStudent(id, studentData.xh);
                }
            }
            finally {
        ...
        // 判断课程当前状态
        // 使用工具函数计算课程时间和状态
        const { courseStartTime, courseEndTime, courseStatus } = calculateCourseTimeAndStatus(attendanceRecord.rq, attendanceRecord.sj_f, attendanceRecord.sj_t);
        // 构建响应数据
        const response = {
            success: true,
            data: {
 ...
 * 教师查看学生打卡详情接口
 */
const getTeacherAttendanceRecord = async (request, reply) => {
    try {
        const { id } = request.params;
        ...
        const stats = await studentAttendanceRepo.getAttendanceStats(id);
        // 获取所有学生的打卡详情
        const studentDetails = await studentAttendanceRepo.getAttendanceRecordWithStudentDetails(id);
        ...
        const response = {
            ...
            data: {
                ...
                student_details: studentDetails.map((student) => {
                    // 计算学生的实际考勤状态
 ...
 * 学生签到接口
 */
const studentCheckIn = async (request, reply) => {
    try {
        ...
        // 获取考勤记录
        const attendanceRecord = await attendanceRepo.getAttendanceRecord(attendance_record_id);
        if (!attendanceRecord) {
            return reply.code(404).send({
                success: false,
                message: '未找到考勤记录'
        ...
        // 检查是否已经签到
        const existingAttendance = await studentAttendanceRepo.getStudentAttendanceByRecordAndStudent(attendance_record_id, studentInfo.xh);
        ...
        // 创建课程服务实例获取授课教师信息
        const courseService = request.diScope.resolve('courseService');
        const primaryTeacher = await courseService.getPrimaryCourseTeacher(attendance_record_id);
 ...
 * 学生请假接口
 */
const studentLeave = async (request, reply) => {
    try {
        const { attendance_record_id, leave_reason, leave_type, attachments } = request.body;
        // 从DI容器中获取JWT payload
        const payload = request.diScope.resolve('userPayload');
        // 验证参数
        if (!attendance_record_id) {
        ...
        const courseService = request.diScope.resolve('courseService');
        // 获取所有授课教师信息
        const allTeachers = await courseService.getCourseTeachers(attendance_record_id);
        ...
        if (allTeachers.length > 0) {
            try {
                ...
                await leaveApprovalRepo.batchCreateLeaveApprovals(newLeaveRecord.id, teacherApprovals);
        ...
        if (attachments && attachments.length > 0) {
            try {
                // 转换附件数据
                const attachmentParams = attachments.map((attachment) => ({
                ...
                const leaveAttachmentRepo = request.diScope.resolve('leaveAttachmentRepo');
                // 批量创建附件
                ...
                request.log.error('请假附件上传失败:', {
                    error: attachmentError instanceof Error
 ...
 * 学生查询请假审批记录接口
 */
const getStudentLeaveApplications = async (request, reply) => {
    try {
        const { status = 'all', page = 1, page_size = 20, start_date, end_date } = request.query;
        // 从DI容器中获取JWT payload
        const payload = request.diScope.resolve('userPayload');
        if (!payload.userId) {
            return reply.code(400).send({
        ...
        // 查询请假申请记录
        const { applications, total } = await studentAttendanceRepo.getStudentLeaveApplications(studentInfo.xh, status, startDate, endDate, limit, offset);
        // 获取统计信息
        const stats = await studentAttendanceRepo.getStudentLeaveApplicationStats(studentInfo.xh, startDate, endDate);
 ...
 * 教师查询请假单接口
 */
const getTeacherLeaveApplications = async (request, reply) => {
    try {
        const { status, page = 1, page_size = 20, start_date, end_date } = request.query;
        // 从DI容器中获取JWT payload
        const payload = request.diScope.resolve('userPayload');
        if (!payload.userId) {
            return reply.code(400).send({
        ...
        if (status) {
            if (Array.isArray(status)) {
                statusFilter = status.filter((s) => ['pending', 'approved', 'rejected', 'cancelled'].includes(s));
            }
            else if (typeof status === 'string') {
                // 支持逗号分隔的状态字符串
                const statusArray = status.split(',').map((s) => s.trim());
                statusFilter = statusArray.filter((s) => ['pending', 'approved', 'rejected', 'cancelled'].includes(s));
            }
        }
        // 获取教师的所有审批记录
        const allApplications = await leaveApprovalRepo.getTeacherApprovals(teacherInfo.gh);
        // 添加调试日志
        request.log.info('教师审批记录查询结果:', {
            teacherId: teacherInfo.gh,
            totalApplications: allApplications.length,
            statusFilter,
            sampleApplications: allApplications.slice(0, 2).map((app) => ({
                approval_id: app.id,
        ...
        if (statusFilter && statusFilter.length > 0) {
            applications = allApplications.filter((app) => statusFilter.includes(app.approval_result));
        }
        // 添加过滤后的调试日志
        ...
        // 获取统计信息
        const stats = await leaveApprovalRepo.getTeacherApprovalStats(teacherInfo.gh);
        // 应用日期过滤
        ...
        const response = {
            ...
            data: {
                ...
                stats: {
                    ...
                    processed_count: stats.approved_count + stats.rejected_count + stats.cancelled_count,
 ...
 * 教师审批请假单接口
 */
const teacherApproveLeave = async (request, reply) => {
    try {
        ...
        const leaveApprovalRepo = request.diScope.resolve('leaveApprovalRepo');
        // 验证审批权限
        const approval = await leaveApprovalRepo.getApprovalByIdAndApprover(approval_id, teacherInfo.gh);
        if (!approval) {
        ...
        // 执行审批操作
        const result = await leaveApprovalRepo.processApproval(approval_id, action, comment);
 ...
 * 查看请假申请附件接口
 */
const viewLeaveAttachment = async (request, reply) => {
    try {
        ...
        // 创建附件Repository实例
        const leaveAttachmentRepo = new LeaveAttachmentRepository(request.diScope.resolve('db'), request.log);
        // 查询附件信息
        const attachment = await leaveAttachmentRepo.getLeaveAttachmentById(attachmentId);
        if (!attachment) {
            return reply.code(404).send({
                success: false,
                message: '附件不存在'
 ...
 * 下载请假申请附件接口
 */
const downloadLeaveAttachment = async (request, reply) => {
    try {
        const { attachmentId } = request.params;
        // 从DI容器中获取JWT payload
        const payload = request.diScope.resolve('userPayload');
 ...
 */
const getLeaveAttachmentImage = async (request, reply) => {
    try {
        const { attachmentId } = request.params;
        // 从DI容器中获取JWT payload
        const payload = request.diScope.resolve('userPayload');
 ...
 * 获取课程历史考勤数据接口
 */
const getCourseAttendanceHistory = async (request, reply) => {
    try {
        ...
        // 获取课程历史考勤数据
        const historyData = await studentAttendanceRepo.getCourseAttendanceHistory(kkh, xnxq, startDate, endDate);
 ...
 * 获取个人课程统计接口
 */
const getPersonalCourseStats = async (request, reply) => {
    try {
        const { kkh } = request.params;
        ...
        const studentAttendanceRepo = request.diScope.resolve('studentAttendanceRepo');
        // 获取个人课程统计数据
        const statsData = await studentAttendanceRepo.getPersonalCourseStats(kkh, xnxq);
 ...
 * 学生撤回请假申请接口
 */
const studentWithdrawLeave = async (request, reply) => {
    try {
        ...
        const studentAttendanceRepo = request.diScope.resolve('studentAttendanceRepo');
        // 执行撤回操作
        const withdrawResult = await studentAttendanceRepo.withdrawStudentLeaveApplication(studentInfo.xh, attendance_record_id);
 ...
 * 注册考勤相关路由
 */
export async function attendanceController(fastify) {
