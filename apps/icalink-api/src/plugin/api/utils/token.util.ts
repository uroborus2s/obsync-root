...
export class CheckInTokenUtil {
  ...
  generateToken(payload: CheckInTokenPayload): string {
    try {
      ...
      throw new Error(
        `生成签到令牌失败: ${error instanceof Error ? error.message : '未知错误'}`
      );
    }
  }
  ...
  verifyToken(token: string): CheckInTokenPayload {
    try {
      ...
      // 解析JSON
      const payload: CheckInTokenPayload = JSON.parse(decrypted);
      ...
      throw new Error(
        `验证签到令牌失败: ${error instanceof Error ? error.message : '未知错误'}`
      );
    }
  }
  ...
  generateSimpleToken(
    ...
    return this.generateToken(payload as CheckInTokenPayload);
  }
