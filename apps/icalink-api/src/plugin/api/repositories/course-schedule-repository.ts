...
export class CourseScheduleRepository extends BaseRepository {
   ...
   * 根据学年学期查询课表数据
   */
  async findByXnxq(xnxq: string): Promise<CourseScheduleEntity[]> {
    try {
   ...
   * 根据开课号查询课表数据
   */
  async findByKkh(kkh: string): Promise<CourseScheduleEntity[]> {
    try {
      const results = await this.db
        .selectFrom('u_jw_kcb_cur')
        .selectAll()
   ...
   */
  async findUnsyncedData(xnxq?: string): Promise<CourseScheduleEntity[]> {
    try {
      let query = this.db
        .selectFrom('u_jw_kcb_cur')
        .selectAll()
   ...
   * 查询变化的数据（用于增量同步）
   */
  async findChangedData(xnxq: string): Promise<CourseScheduleEntity[]> {
    try {
      const results = await this.db
        .selectFrom('u_jw_kcb_cur')
        .selectAll()
   ...
   * 清空指定学年学期的同步状态
   */
  async resetSyncStatus(xnxq: string): Promise<number> {
    try {
      const result = await this.db
        .updateTable('u_jw_kcb_cur')
        .set({
   ...
   * 获取同步状态统计
   */
  async getSyncStats(xnxq: string): Promise<{
    total: number;
    synced: number;
    unsynced: number;
    deleted: number;
