...
export class StudentAttendanceRepository extends BaseRepository {
    db;
    studentCourseRepo;
    constructor(db, log, studentCourseRepo) {
        super(log);
        this.db = db;
    ...
    async getAttendanceStats(attendanceRecordId) {
        try {
            ...
            const totalCount = await this.studentCourseRepo.getStudentCountByKkh(attendanceRecord.kkh);
            // 获取已有签到记录的统计
    ...
    async getAttendanceRecordWithStudentDetails(attendanceRecordId) {
        try {
            ...
            // 查询这门课的所有学生（从学生课表中获取）
            const courseStudents = await this.studentCourseRepo.findStudentsWithDetailsByCourse(attendanceRecord.kkh, attendanceRecord.xnxq);
            // 查询已有的签到记录
