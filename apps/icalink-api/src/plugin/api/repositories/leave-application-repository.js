...
export class LeaveApplicationRepository extends BaseRepository {
     ...
     * 根据学生考勤记录ID获取请假申请
     */
    async getLeaveApplicationByAttendanceRecordId(attendanceRecordId) {
        try {
            ...
            this.log.debug({ attendanceRecordId, studentId: result.student_id }, '获取请假申请成功');
            return result;
        }
        catch (error) {
            this.log.error({
     ...
     * 根据条件查询请假申请
     */
    async findLeaveApplications(conditions, limit, offset) {
        try {
            ...
            if (conditions.application_time_start) {
                query = query.where('application_time', '>=', conditions.application_time_start);
            }
            if (conditions.application_time_end) {
                query = query.where('application_time', '<=', conditions.application_time_end);
            }
            // 添加排序
     ...
     * 更新请假申请
     */
    async updateLeaveApplication(id, updates) {
        try {
     ...
     * 批准请假申请
     */
    async approveLeaveApplication(id) {
        try {
     ...
     * 拒绝请假申请
     */
    async rejectLeaveApplication(id) {
        try {
     ...
     * 获取请假申请统计
     */
    async getLeaveApplicationStats(conditions) {
        try {
     ...
     * 获取学生的请假申请历史
     */
    async getStudentLeaveApplicationHistory(studentId, limit = 50, offset = 0) {
        try {
            ...
            const results = await this.db
                ...
                .offset(offset)
                .execute();
            this.log.debug({ studentId, count: results.length }, '获取学生请假申请历史完成');
            return results;
        }
        catch (error) {
            this.log.error({
     ...
     * 获取教师需要审批的请假申请
     */
    async getTeacherPendingApplications(teacherId, limit = 50, offset = 0) {
        try {
            ...
            const results = await this.db
                ...
                .offset(offset)
                .execute();
            this.log.debug({ teacherId, count: results.length }, '获取教师待审批请假申请完成');
            return results;
        }
        catch (error) {
            this.log.error({
     ...
     * 检查学生是否已有相同课程的请假申请
     */
    async hasExistingLeaveApplication(studentId, kkh, classDate) {
        try {
            this.log.debug({ studentId, kkh, classDate }, '检查是否存在相同课程的请假申请');
            const result = await this.db
                .selectFrom('icalink_leave_applications')
                .select('id')
                .where('student_id', '=', studentId)
     ...
     * 批量创建请假申请
     */
    async batchCreateLeaveApplications(applications) {
        try {
...
//# sourceMappingURL=leave-application-repository.js.map