...
import { TaskTreeTableData } from './task-tree-table-new'

interface TaskTreeRowActionsProps {
  row: Row<TaskTreeTableData>
}

export function TaskTreeRowActions({ row }: TaskTreeRowActionsProps) {
  ...
  const canRetry = [TaskStatus.FAILED, TaskStatus.CANCELLED].includes(
    task.status
  )
  const canDelete = [
    TaskStatus.SUCCESS,
