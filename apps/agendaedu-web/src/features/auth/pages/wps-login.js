...
import { AlertCircle, CheckCircle2, QrCode, RefreshCw, Smartphone, } from 'lucide-react';
        ...
        try {
            ...
            if (window.qrcode && typeof window.qrcode.generateQrCode === 'function') {
                ...
                const checkInterval = setInterval(() => {
                    attempts++;
                    const qrCodeElement = document.querySelector('.kso-qr-code-sdk');
      ...
      <div className='w-full max-w-md rounded-2xl bg-white p-8 shadow-xl'>
        ...
        <div className='mb-8'>
          <div className='relative flex min-h-[300px] items-center justify-center rounded-xl bg-gray-50 p-6'>
            ...
            {(status === 'ready' || status === 'scanned') && (<div ref={qrContainerRef} className='flex h-full w-full items-center justify-center'/>)}
        ...
        {/* 操作按钮 */}
        {status === 'error' && (<div className='flex space-x-3'>
            <Button onClick={refreshQrCode} className='flex-1'>
              <RefreshCw className='mr-2 h-4 w-4'/>
              重新加载
            </Button>
            <Button onClick={() => navigate({ to: '/sign-in' })} variant='outline' className='flex-1'>
              其他方式登录
            </Button>
          </div>)}

        {status === 'ready' && (<div className='space-y-3'>
            <Alert>
...
//# sourceMappingURL=wps-login.js.map