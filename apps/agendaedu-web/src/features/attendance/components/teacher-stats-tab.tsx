...
export function TeacherStatsTab() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async (): Promise<TeacherStatsResponse> => {
      ...
      const response = await fetch(
        `/api/icalink/v1/attendance/stats/teachers?${params}`
      )
      if (!response.ok) {
        throw new Error('获取教师统计数据失败')
      }
        ...
        <div className='space-y-2'>
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value='course_count'>课程数</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className='text-center'>授课课程数</TableHead>
              <TableHead className='text-center'>总课次数</TableHead>
          ...
          <TableBody>
            {isLoading ? (
              ...
              data?.data.map((teacher) => (
                <TableRow key={teacher.teacher_code}>
                  <TableCell>
                    <div>
                  ...
                  <TableCell className='text-center'>
                    {teacher.total_should_attend}
                  </TableCell>
                  <TableCell className='text-center'>
                    {teacher.actual_attended}
                  </TableCell>
                  <TableCell className='text-center'>
                    {teacher.leave_count}
                  </TableCell>
                  ...
                  <TableCell className='text-center'>
                    {getAttendanceRateBadge(teacher.attendance_rate)}
                  </TableCell>
                </TableRow>
              ))
            )}
