...
import { Calendar, CheckCircle, Clock, Search, TrendingUp, User, } from 'lucide-react';
...
export function StudentAttendanceTab() {
    ...
    const filteredStudents = students.filter((student) => student.student.xm.toLowerCase().includes(searchQuery.toLowerCase()) ||
        student.student.xh.includes(searchQuery));
    return (<div className='space-y-6'>
      {/* 搜索 */}
      <div className='flex gap-4'>
        <Input placeholder='搜索学生姓名或学号...' value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className='max-w-sm'/>
      ...
      <div className='grid gap-6 lg:grid-cols-2'>
        ...
        <Card>
          ...
          <CardContent>
            {loading ? (<div className='py-8 text-center'>
                ...
                {filteredStudents.map((student) => (<Card key={student.student.xh} className={`hover:bg-muted/50 cursor-pointer transition-colors ${selectedStudent?.student.xh === student.student.xh
                    ? 'ring-primary ring-2'
                      ...
                      <div className='flex items-center gap-4'>
                        ...
                        <div className='flex-1'>
                          ...
                          <div className='mt-2 text-sm'>
                            <span className={`font-semibold ${getAttendanceRateColor(student.attendance_rate)}`}>
                              出勤率:{' '}
                              {(student.attendance_rate * 100).toFixed(1)}%
                            </span>
                          </div>
                          <Progress value={student.attendance_rate * 100} className='mt-2 h-2'/>
                        </div>
                        <div className='text-right'>
                          ...
                          {student.recent_record && (<div className='mt-2 flex items-center gap-1 text-xs'>
        ...
        <Card>
          <CardHeader>
            ...
            {selectedStudent && (<CardDescription>
            ...
            {selectedStudent ? (<div className='space-y-6'>
                ...
                <div className='grid gap-4 md:grid-cols-2'>
                  ...
                  <Card>
                    <CardContent className='pt-4'>
                      ...
                      <div className={`mt-1 text-2xl font-bold ${getAttendanceRateColor(selectedStudent.attendance_rate)}`}>
                ...
                <div>
                  ...
                  {studentRecords.length > 0 ? (<Table>
                      ...
                      <TableBody>
                        {studentRecords.map((record) => (<TableRow key={record.id}>
                    ...
                    </Table>) : (<div className='text-muted-foreground py-8 text-center'>
                      暂无考勤记录
                    </div>)}
                </div>
              </div>) : (<div className='text-muted-foreground py-12 text-center'>
...
//# sourceMappingURL=student-attendance-tab.js.map