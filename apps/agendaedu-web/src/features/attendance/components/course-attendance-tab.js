...
export function CourseAttendanceTab() {
    ...
    const filteredCourses = courses.filter((course) => course.kcmc.toLowerCase().includes(searchQuery.toLowerCase()) ||
        course.kkh.includes(searchQuery));
    const getStatusBadge = (status) => {
        return status === 'active' ? (<Badge variant='default'>进行中</Badge>) : (<Badge variant='secondary'>已结束</Badge>);
    };
    ...
    return (<div className='space-y-6'>
      ...
      <div className='flex gap-4'>
        <Input placeholder='搜索课程名称或开课号...' value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className='max-w-sm'/>
          ...
          <Card>
            ...
            <CardContent>
              {loading ? (<div className='py-8 text-center'>
                ...
                const attendanceRate = course.total_count > 0
                ...
                return (<TableRow key={course.id}>
                          ...
                          <TableCell>
                            <div className='space-y-1'>
                              ...
                              {course.leave_count > 0 && (<Badge variant='secondary' className='ml-1'>
                                  请假: {course.leave_count}
                                </Badge>)}
                              {course.absent_count > 0 && (<Badge variant='destructive' className='ml-1'>
                          ...
                          <TableCell>
                            <Button variant='outline' size='sm' onClick={() => fetchCourseDetail(course.id)}>
          ...
          {selectedCourse && (<Card>
              ...
              <CardContent>
                ...
                <Table>
                  ...
                  <TableBody>
                    {selectedCourse.student_attendances.map((attendance) => (<TableRow key={attendance.id}>
                        <TableCell>{attendance.student_id}</TableCell>
                        <TableCell>{attendance.student_name}</TableCell>
                        <TableCell>
                          <Badge variant={attendance.status === 'present'
                    ? 'default'
...
//# sourceMappingURL=course-attendance-tab.js.map