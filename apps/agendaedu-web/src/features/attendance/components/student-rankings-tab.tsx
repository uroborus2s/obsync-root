...
export function StudentRankingsTab() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async (): Promise<RankingsResponse> => {
      ...
      if (!response.success) {
        throw new Error(response.message || '获取学生排行榜数据失败')
      }
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">参与排名学生</CardTitle>
          ...
          <TableBody>
            {isLoading ? (
              ...
              data?.data.map((student) => (
                <TableRow key={student.id} className={getRowClassName(student.rank)}>
                  ...
                  <TableCell>
                    <div className="text-sm">{student.extra_info || '-'}</div>
                  </TableCell>
                  <TableCell className="text-center">{student.total_count}</TableCell>
                  <TableCell className="text-center">
      ...
      {data && data.total > filters.page_size && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-muted-foreground">
            显示前 {Math.min(filters.page_size, data.total)} 名，共 {data.total} 名学生
          </div>
