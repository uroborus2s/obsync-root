...
import { BarChart3, BookOpen, Download, RefreshCw, TrendingUp, Trophy, Users, } from 'lucide-react';
...
export function StatsAttendanceTab() {
    ...
    const getRankingBadge = (index) => {
        ...
        if (index === 2)
            return <Badge className='bg-amber-600'>🥉</Badge>;
      ...
      <Tabs defaultValue='overview' className='space-y-4'>
        ...
        <TabsContent value='overview' className='space-y-4'>
          ...
          {loading ? (<div className='py-8 text-center'>
            ...
            </div>) : stats ? (<div className='grid gap-6 md:grid-cols-2'>
              <Card>
                <CardHeader>
                ...
                <CardContent className='space-y-4'>
                  ...
                  <div className='space-y-2'>
                    <div className='flex items-center justify-between'>
                      ...
                      <span className={`font-bold ${getAttendanceRateColor(stats.average_attendance_rate)}`}>
                        {(stats.average_attendance_rate * 100).toFixed(1)}%
                      </span>
                    </div>
                    <Progress value={stats.average_attendance_rate * 100} className='h-2'/>
                  </div>
                ...
                <CardContent className='space-y-6'>
                  <div className='space-y-4'>
                    <div className='flex items-center justify-between'>
                      ...
                      <Badge variant={stats.total_courses > 0 ? 'default' : 'secondary'}>
                    ...
                    <div className='flex items-center justify-between'>
                      ...
                      <Badge variant={stats.average_attendance_rate >= 0.9
                ? 'default'
                : 'destructive'}>
          ...
          <Card>
            ...
            <CardContent>
              {ranking.length > 0 ? (<Table>
                  ...
                  <TableBody>
                    {ranking.map((student, index) => (<TableRow key={student.student.xh}>
                        ...
                        <TableCell>
                          <div className='space-y-1'>
                            ...
                            {student.leave_count > 0 && (<Badge variant='secondary' className='ml-1'>
                                请假: {student.leave_count}
                              </Badge>)}
                            {student.absent_count > 0 && (<Badge variant='destructive' className='ml-1'>
                        ...
                        <TableCell>
                          <div className='space-y-1'>
                            <div className={`font-bold ${getAttendanceRateColor(student.attendance_rate)}`}>
                              {(student.attendance_rate * 100).toFixed(1)}%
                            </div>
                            <Progress value={student.attendance_rate * 100} className='h-1 w-16'/>
                          </div>
                        </TableCell>
                        <TableCell>
                          <Badge variant={student.attendance_rate >= 0.9
                    ? 'default'
                    : 'destructive'}>
...
//# sourceMappingURL=stats-attendance-tab.js.map