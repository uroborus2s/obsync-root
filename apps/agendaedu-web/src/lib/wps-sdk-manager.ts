/**
 * WPS官方SDK管理器
 * 基于WPS官方文档实现: https://open-xz.wps.cn/pages/general/app-login/Web-app-login/SDK-access-document/
 */
import type { AuthUrlParams, GenerateQrCodeParams } from '@/types/wps-sdk'
...
export class WPSSDKManager {
  ...
  generateQrCode(state?: string): void {
    ...
    try {
      ...
      throw new Error(
        `WPS SDK调用失败: ${error instanceof Error ? error.message : '未知错误'}`
      )
    }
  }
   ...
   * 等待SDK加载完成
   */
  waitForSDK(timeout = 15000): Promise<void> {
  ...
  private getDiagnosticInfo(): string {
    ...
    info.push(
      `- window.qrcode.generateQrCode: ${typeof window.qrcode?.generateQrCode}`
    )

    // 检查脚本加载
    const scripts = Array.from(document.scripts)
    const wpsScript = scripts.find(
      (script) =>
        script.src.includes('wpscdn.cn') || script.src.includes('qrcode')
    )
    ...
    if (wpsScript) {
      ...
      const readyState = (
        wpsScript as HTMLScriptElement & { readyState?: string }
      ).readyState
