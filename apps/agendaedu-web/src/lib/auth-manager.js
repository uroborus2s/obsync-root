...
export class WpsAuthManager {
    ...
    async getUserInfo() {
        ...
        if (!token)
            return null;
        try {
            const response = await fetch(`${this.config.baseUrl}/oauthapi/v3/user/info`, {
                headers: {
     ...
     * 处理SDK登录成功（保持向后兼容）
     */
    async handleSDKLoginSuccess(data) {
        try {
            if (!data.code) {
            ...
            // 触发登录成功事件
            window.dispatchEvent(new CustomEvent('wps-login-success', {
            ...
            window.dispatchEvent(new CustomEvent('wps-login-error', {
                detail: {
    ...
    initSDKListeners() {
        // 监听WPS SDK登录成功事件
        window.addEventListener('wps-login-success', (event) => {
