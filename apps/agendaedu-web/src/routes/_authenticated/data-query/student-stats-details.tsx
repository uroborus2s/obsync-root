...
import {
  StatsQueryParams,
  StudentOverallAttendanceStatsDetails,
  statsApiService,
} from '@/lib/stats-api'
...
export const Route = createFileRoute(
  ...
  component: StudentStatsDetailsPage,
})

function StudentStatsDetailsPage() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['student-stats-details', filters],
    queryFn: () => statsApiService.getStudentStatsDetails(filters),
    ...
    <div className='flex-1 space-y-6 p-4 pt-6 md:p-8'>
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-muted-foreground'>查询学生课程级别出勤统计详情</p>
        </div>
      </div>
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  records.map(
                    (
                      record: StudentOverallAttendanceStatsDetails,
                      index: number
                    ) => (
                      <TableRow
                        key={`${record.student_id}-${record.course_code}-${record.teaching_week}-${index}`}
                      >
