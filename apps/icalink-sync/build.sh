...
# 默认配置
DEFAULT_PROJECT_NAME="icalink-sync"
...
# 显示帮助信息
show_help() {
    echo -e "${BLUE}iCalink Sync Docker 构建和推送脚本${NC}"
    echo ""
    echo "用法:"
    ...
    echo "示例:"
    echo "  $0                                    # 使用默认参数"
    echo "  $0 icalink-sync v1.0.0               # 指定项目和版本"
    echo "  $0 --no-cache icalink-sync v1.0.0    # 禁用缓存构建"
    echo "  $0 --dry-run icalink-sync v1.0.0     # 预览命令"
    echo ""
}
...
check_environment() {
    ...
    if [ ! -f "package.json" ] || [ ! -f "dockerfile" ] || [ ! -f ".npmrc" ]; then
        log_error "请在 icalink-sync 项目根目录下运行此脚本"
...
main() {
    ...
    echo -e "${BLUE}🚀 iCalink Sync Docker 构建和推送${NC}"
    echo "======================================"
