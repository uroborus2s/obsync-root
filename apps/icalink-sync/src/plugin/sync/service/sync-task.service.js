...
export class SyncTaskService {
    taskTreeService;
    log;
    constructor(taskTreeService, log) {
     ...
     * 恢复同步任务
     */
    async resumeSync(taskId, reason) {
        try {
     ...
     * 取消同步任务
     */
    async cancelSync(taskId, reason) {
        try {
