...
export class CleanupDataExecutor {
    ...
    async execute(taskNode, context) {
 ...
 * 重新聚合数据任务执行器
 */
export class RegenerateAggregateExecutor {
    db;
    courseAggregateRepo;
    log;
    name = 'regenerate_aggregate';
    constructor(db, courseAggregateRepo, log) {
        this.db = db;
    ...
    async generateAggregateData(xnxq, period) {
    ...
    `;
        ...
        this.log.debug({ xnxq, period }, `${period === 'am' ? '上午' : '下午'}时段聚合完成`);
    }
}
/**
 * 签到表创建任务执行器
 */
export class CreateAttendanceTableExecutor {
    attendanceService;
    db;
    log;
    name = 'createAttendanceTableExecutor';
    constructor(attendanceService, db, log) {
        this.attendanceService = attendanceService;
        this.db = db;
        this.log = log;
    }
    async onStart(taskNode, context) {
        ...
        try {
            this.log.info({ courseKkh: courseTask.kkh, totalStudentCount }, '获取课程学生总数');
            // 3. 构建CourseInfo对象
            const courseInfo = {
                kkh: courseTask.kkh,
            ...
            const attendanceRecord = await this.attendanceService.createAttendanceRecord(taskId, courseInfo, totalStudentCount);
            this.log.info({
            ...
            taskNode.success();
            this.log.info({ courseKkh: courseTask.kkh }, '签到表创建任务onStart执行成功');
        }
        catch (error) {
            this.log.error({
                error: error instanceof Error ? error.message : String(error),
    ...
    async onSuccess(taskNode, context) {
        const { courseTask, taskType } = taskNode.data.metadata;
        this.log.info({ courseKkh: courseTask.kkh, courseName: courseTask.kcmc, taskType }, '签到表记录创建任务成功完成');
        try {
            // 根据任务类型更新 juhe_renwu 表的 gx_zt 状态
            if (taskType === 'teacher') {
                // 教师任务完成，设置 gx_zt = 1
                await this.updateAggregateStatus(courseTask, 1);
                this.log.info({ courseKkh: courseTask.kkh }, '教师任务完成，juhe_renwu表gx_zt已更新为1');
            }
            else if (taskType === 'student') {
                // 学生任务完成，设置 gx_zt = 2
                await this.updateAggregateStatus(courseTask, 2);
                this.log.info({ courseKkh: courseTask.kkh }, '学生任务完成，juhe_renwu表gx_zt已更新为2');
    ...
    async onFail(taskNode, context) {
        const { courseTask, taskType } = taskNode.data.metadata;
        this.log.error({ courseKkh: courseTask.kkh, courseName: courseTask.kcmc, taskType }, '签到表记录创建任务失败');
    }
    async onComplete(taskNode, context) {
        const { courseTask, taskType } = taskNode.data.metadata;
        this.log.info({ courseKkh: courseTask.kkh, courseName: courseTask.kcmc, taskType }, '签到表记录创建任务已完成');
    }
    /**
     * 更新聚合表状态
     */
    async updateAggregateStatus(courseTask, gxZt) {
        try {
            await this.db
     ...
     */
    async checkAndUpdateCourseStatus(courseTask) {
        try {
 ...
 * 教师日程同步任务执行器
 */
export class SyncTeacherScheduleExecutor {
    queueService;
    userCalendarRepo;
    taskTreeService;
    wasV7Schedule;
    ...
    constructor(queueService, userCalendarRepo, // UserCalendarRepository
    taskTreeService, wasV7Schedule, wasV7Calendar, log) {
    ...
    async onStart(taskNode, context) {
        if (taskNode.data.status !== TaskStatus.PENDING &&
            taskNode.data.status !== TaskStatus.RUNNING) {
            this.log.info({ taskId: taskNode.id }, '教师日程同步任务已存在，跳过日程同步');
            return;
        }
        const { courseTask, userId, type, userName, jobId, taskId } = taskNode.data.metadata;
        this.log.info({
            taskId: taskNode.id,
            userId: userId,
            userName: userName,
        ...
        try {
            ...
            let userCalendar = await this.userCalendarRepo.findByXgh(userId);
            if (!userCalendar) {
                // 调用wps接口创建日程，并将日程写入到userCalendarRepo数据库中
                const wpsCalendar = await this.wasV7Calendar.createCalendar({
                    summary: `${userName}的日历`
                });
                // 写入userCalendarRepo 库中
                userCalendar = await this.userCalendarRepo.create({
                    xgh: userId,
                    name: userName,
                    calendar_id: wpsCalendar.id
                });
                this.log.warn({ teacherId: userId, teacherName: userName }, '教师未找到日历信息，跳过日程同步');
            }
            if (!userCalendar.calendar_id) {
                this.log.warn({ teacherId: userId, teacherName: userName }, '教师日历ID为空，跳过日程同步');
                // 直接标记任务完成，但记录为跳过状态
                await this.taskTreeService.fail(taskNode.id, '教师日历ID为空，跳过日程同步', new Error(`教师日历ID为空，跳过日程同步`));
                return;
            }
            this.log.info({
                teacherId: userId,
            ...
            try {
                ...
                if (schedules.length > 1) {
                    ...
                    if (schedules[0].id && schedules[0].summary === courseTask.kcmc) {
                        this.log.info({
                            teacherId: userId,
                            teacherName: userName,
 ...
 * 生成考勤记录任务执行器
 */
export class GenerateAttendanceExecutor {
    courseAggregateRepo;
    log;
    ...
    async execute(taskNode, context) {
        ...
        try {
            ...
            const attendanceCourses = []; // 暂时使用空数组
            this.log.info({ xnxq, attendanceCourseCount: attendanceCourses.length }, '找到需要考勤的课程');
            // TODO: 实现考勤记录生成逻辑
...
//# sourceMappingURL=task-executors.service.js.map