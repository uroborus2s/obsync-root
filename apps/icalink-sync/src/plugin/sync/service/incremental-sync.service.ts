...
export class IncrementalSyncService {
  ...
  async startIncrementalSync(
    ...
    try {
      ...
      this.log.info(
        { changedCount: changedData.length },
        '获取到未处理的变化数据'
      );
      ...
      // 步骤 6: 删除教师和学生日历对象
      const { teacherCount, studentCount } = await this.deleteUserCalendars(
        changedData,
        xnxq
      );
      ...
      this.log.info(
        {
          ...
          deletedCalendars:
            stats.deletedTeacherCalendars + stats.deletedStudentCalendars,
   ...
   * 获取变化数据的 kkh 和 rq
   */
  private async getChangedKkhAndRq(
    xnxq: string
  ): Promise<Array<{ kkh: string; rq: string }>> {
    try {
      ...
      // 按 kkh 和 rq 分组去重
      const changedData =
        await this.courseScheduleRepo.getChangedKkhAndRq(xnxq);

      this.log.info(
        { xnxq, count: changedData.length },
        '获取变化的开课号和日期完成'
      );
   ...
   */
  private async softDeleteAggregateData(
    changedData: Array<{ kkh: string; rq: string }>
  ): Promise<number> {
    try {
   ...
   */
  private async deleteUserCalendars(
    changedData: Array<{ kkh: string; rq: string }>,
    xnxq: string
    ...
    try {
      ...
      for (const { kkh, rq } of changedData) {
        // 1. 删除教师日历
        const teacherDeleteCount = await this.deleteTeacherCalendarsForCourse(
          kkh,
          rq,
          xnxq
        );
        ...
        // 2. 删除学生日历
        const studentDeleteCount = await this.deleteStudentCalendarsForCourse(
          kkh,
          rq,
          xnxq
        );
   ...
   * 删除指定课程的教师日历
   */
  private async deleteTeacherCalendarsForCourse(
    kkh: string,
    rq: string,
    xnxq: string
  ): Promise<number> {
    try {
      ...
      const teacherXghs = [
        ...new Set(
          courseSchedules.map((schedule) => schedule.ghs).filter(Boolean)
        )
      ];
   ...
   * 删除指定课程的学生日历
   */
  private async deleteStudentCalendarsForCourse(
    kkh: string,
    rq: string,
    xnxq: string
  ): Promise<number> {
   ...
   * 把新聚合的值附加到聚合表
   */
  private async regenerateAggregateData(xnxq: string): Promise<number> {
    try {
      this.log.debug({ xnxq }, '开始重新聚合数据');

      // 使用 CourseScheduleRepository 的方法生成增量聚合数据
      const aggregateStats =
        await this.courseScheduleRepo.generateIncrementalAggregateData(xnxq);
   ...
   * 获取增量同步状态
   */
  async getIncrementalSyncStatus(xnxq: string): Promise<{
