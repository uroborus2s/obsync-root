 ...
 */
export class WpsScheduleProcessorService {
    wasV7Schedule;
    taskTreeService;
    pageUrlService;
    log;
    name = 'wpsScheduleProcessorService';
    constructor(wasV7Schedule, taskTreeService, pageUrlService, // 注入TaskTreeService用于任务状态回调
    log) {
    ...
    async validate(payload) {
        ...
        if (!payload.participantType ||
            ...
            this.log.error({ participantType: payload.participantType }, '无效的参与者类型');
            return false;
        }
     ...
     * 创建WPS日程 - 统一入口方法
     */
    async createSchedule(taskData) {
        return await this.processCreateScheduleTask(taskData);
    }
    /**
     * 处理任务回调 - 更新原始任务状态
     */
    async handleTaskCallback(metadata, success, error) {
        ...
        try {
            ...
            switch (action) {
                case 'complete_task':
                    await this.taskTreeService.success(taskId, reason || '队列任务执行成功', { queueJobSuccess: true });
                    this.log.info({ parentTaskId: taskId, reason }, '原始任务已标记为完成');
                    break;
                case 'fail_task':
                    await this.taskTreeService.fail(taskId, reason || error || '队列任务执行失败', error ? new Error(error) : undefined);
                    this.log.info({ parentTaskId: taskId, reason, error }, '原始任务已标记为失败');
                    break;
                default:
     ...
     * 处理WPS日程创建任务
     */
    async processCreateScheduleTask(taskData) {
        const { participantType, participantId, participantName, calendarId, courseData } = taskData;
        this.log.info({
            participantType,
            participantId,
            participantName,
        ...
        try {
            ...
            // 调用WPS API创建日程
            const scheduleResult = await this.wasV7Schedule.createSchedule(scheduleParams);
            this.log.info({
                participantType,
                participantId,
                participantName,
     ...
     * 构建日程描述
     */
    buildScheduleDescription(courseData, participantType) {
        const lines = [
            `教学周: ${courseData.jxz}`,
        ...
        if (participantType === 'student' &&
            ...
            lines.push(`签到地址: ${this.pageUrlService.getuAttendanceUrl(courseData.taskId)}`);
        }
        if (participantType === 'teacher') {
            lines.push(`签到信息: ${this.pageUrlService.getTeacherQueueUrl(courseData.taskId)}`);
        }
        return lines.join('\n');
    }
    /**
    ...
    formatDateTime(date, time) {
        try {
            this.log.debug({ inputDate: date, inputTime: time }, '开始格式化日期时间为RFC3339格式');
    ...
    async processDeleteScheduleTask(taskData) {
        const { calendarId, eventId, participantId, participantName, participantType, courseData } = taskData;
        this.log.info({
            participantType,
            participantId,
            participantName,
...
//# sourceMappingURL=wps-schedule-processor.service.js.map