 ...
 */
export class UpdateAggregateService implements TaskExecutor {
   ...
   */
  private groupCourseSchedules(schedules: any[]): Array<{
    kkh: string;
    xnxq: string;
    jxz: string;
    zc: number;
   ...
   * 用于批量处理多个任务的状态更新
   */
  async batchUpdateAggregateStatus(
    courseTasks: Array<{
      kkh: string;
      xnxq: string;
      rq: string;
   ...
   */
  async softDeleteAggregateTasks(
    conditions: Array<{ kkh: string; rq: string }>
  ): Promise<void> {
    try {
      let totalUpdated = 0;

      for (const condition of conditions) {
        const updated = await this.courseAggregateRepo.softDelete(
          condition.kkh,
          condition.rq
        );
   ...
   */
  async completeSoftDeleteProcessing(xnxq: string): Promise<void> {
    try {
      // 查找所有状态为3的聚合任务
      ...
      const updatedCount = await this.courseAggregateRepo.batchUpdateStatus(
        taskIds,
        4
      );
   ...
   * 获取聚合任务统计信息
   */
  async getAggregateStats(xnxq: string): Promise<{
    total: number;
    pending: number;
   ...
   */
  async resetAggregateStatus(xnxq: string): Promise<void> {
    try {
      // 重置聚合表状态
      ...
      const scheduleResult =
        await this.courseScheduleRepo.resetSyncStatus(xnxq);
   ...
   */
  async execute(taskNode: TaskNode): Promise<void> {
   ...
   */
  async onSuccess(taskNode: TaskNode): Promise<void> {
   ...
   */
  async onFail(taskNode: TaskNode): Promise<void> {
    const { action, courseTask } = taskNode.data.metadata;
   ...
   */
  async onComplete(taskNode: TaskNode): Promise<void> {
    const { action, courseTask } = taskNode.data.metadata;
