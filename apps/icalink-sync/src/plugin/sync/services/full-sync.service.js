 ...
 */
import { TaskNode, TaskStatus } from '@stratix/tasks';
...
export class FullSyncService {
    ...
    constructor(taskTreeService, courseAggregateRepo, studentCourseRepo, studentInfoRepo, pageUrlService, log, syncTeacherScheduleExecutor, wasV7Schedule, createAttendanceTableExecutor, courseScheduleRepo, userCalendarRepo, attendanceService, updateAggregateService, wpsScheduleProcessorService) {
        ...
        this.log = log;
        this.syncTeacherScheduleExecutor = syncTeacherScheduleExecutor;
        this.wasV7Schedule = wasV7Schedule;
        this.createAttendanceTableExecutor = createAttendanceTableExecutor;
        ...
        this.wpsScheduleProcessorService = wpsScheduleProcessorService;
    }
    /**
     * 查找已存在的任务
     */
    findExistingTask(name) {
        try {
    ...
    async incremSyncc(config) {
        const { xnxq, batchSize = 50, parallel = false, maxConcurrency = 5 } = config;
        ...
        for (const juheRenwu of aggregateTasks) {
            await this.updateAggregateService.updateRelatedCourseSchedules([juheRenwu], 3);
            ...
            // 获取关联的学生数据
            const students = await this.getStudentsForCourse(juheRenwu.kkh, juheRenwu.xnxq);
            //将教师的日程信息删除
            for (const teacher of teachers) {
                // 3. 从日历中删除日程
                try {
                    // 获取教师的日历id
                    const userCalendar = await this.userCalendarRepo.findByXgh(teacher.gh);
                    if (!userCalendar) {
                        this.log.warn({ teacherId: teacher.gh, teacherName: teacher.xm }, '教师未找到日历信息，跳过日程同步');
                        continue;
                    }
                    // 检查日程是否存在，如果存在则删除日程
                    ...
                    this.log.error({ error: error instanceof Error ? error.message : String(error) }, '删除日程失败');
                }
            }
            // 删除学生的日程
            for (const student of students) {
                ...
                if (!userCalendar) {
                    this.log.warn({ studentId: student.xh, studentName: student.xm }, '学生未找到日历信息，跳过日程同步');
                    continue;
                }
                if (!userCalendar.calendar_id) {
                    this.log.warn({ studentId: student.xh, studentName: student.xm }, '学生日历ID为空，跳过日程同步');
                    continue;
                }
                // 删除学生的日程
                const schedules = await this.wasV7Schedule.getAllScheduleList({
            ...
            await this.updateAggregateService.updateRelatedCourseSchedules([juheRenwu], 4);
        }
    }
    async incremSyncc2(config) {
        const { xnxq, batchSize = 50, parallel = false, maxConcurrency = 5 } = config;
        await this.courseScheduleRepo.generateAggregateData(xnxq, {
            gxZtFilter: '4',
            zt: true,
     ...
     * 启动全量同步任务
     */
    async startFullSync(config) {
        const { xnxq, batchSize = 50, parallel = false, maxConcurrency = 5 } = config;
        this.log.info({ xnxq, batchSize, parallel, maxConcurrency }, '开始启动全量同步任务');
        try {
            // 1. 检查根任务是否已存在
            ...
            this.log.info({ xnxq, courseCount: aggregateTasks.length }, '获取到聚合课程数据');
            for (const courseTask of aggregateTasks) {
                if (!courseTask.sj_f) {
                    continue;
                }
                const startDate = formatToRFC3339(courseTask.rq, courseTask.sj_f);
                const taskId = Buffer.from(`${xnxq}.${courseTask.kkh}.${startDate}`, 'utf8').toString('base64');
                // // 将taskId转换为base64
                // // 1. 获取课程的教师信息
                // const teachers = await this.getTeachersForCourse(courseTask);
                // 2. 获取课程的学生信息
                const students = await this.getStudentsForCourse(courseTask.kkh, courseTask.xnxq);
                if (!this.pageUrlService.isCheckInUrl(courseTask.sfdk)) {
                    // 创建签到表任务
                    await this.createAttendanceTableTask(taskId, rootTask.id, students.length, courseTask);
                }
                ...
                // 创建教师同步任务
                await this.createTeacherSyncTasks(xnxq, rootTask.id, taskId, teachers, courseTask);
                // 创建学生同步任务
                await this.createStudentSyncTasks(xnxq, rootTask.id, taskId, students, courseTask);
            }
        }
        catch (error) {
            this.log.error({
     ...
     * 创建签到表任务
     */
    async createAttendanceTableTask(taskId, parentTaskId, studentCount, courseTask) {
        // 创建签到任务
        ...
        if (!attendanceTask) {
            ...
            this.log.info({ attendanceTaskName }, '签到表记录任务已经完成，不需要在创建');
        }
        if (attendanceTask instanceof TaskNode &&
            attendanceTask.status === TaskStatus.PENDING) {
            attendanceTask.start('签到表记录创建任务启动');
        }
        setTimeout(() => {
            this.createAttendanceTableExecutor.onStart(attendanceTask, {});
        }, 1000 * 60);
    }
    /**
     * 创建教师日程同步任务
     */
    async createTeacherSyncTasks(xnxq, parentTaskId, taskId, teachers, courseTask) {
        // 1. 创建教师组同步任务
     ...
     * 创建学生日程同步任务
     */
    async createStudentSyncTasks(xnxq, parentTaskId, taskId, students, courseTask) {
     ...
     * 获取课程的教师信息
     */
    async getTeachersForCourse(courseTask) {
     ...
     * 获取课程的学生信息
     */
    async getStudentsForCourse(kkh, xnxq) {
        try {
            // 从学生课表关联表获取学生列表
            const studentCourses = await this.studentCourseRepo.findStudentsByKkhAndXnxq(kkh, xnxq);
     ...
     * 获取全量同步状态
     */
    async getFullSyncStatus(taskId) {
        try {
            const task = this.taskTreeService.getTask(taskId);
            if (!task) {
                return null;
...
//# sourceMappingURL=full-sync.service.js.map