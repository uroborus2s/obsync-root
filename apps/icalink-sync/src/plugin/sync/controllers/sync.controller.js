 ...
 * 启动全量同步
 */
const startFullSync = async (request, reply) => {
    try {
        const { xnxq, batchSize, maxRetries, timeout } = request.body;
        request.log.info({ xnxq, batchSize, maxRetries, timeout }, '收到全量同步请求');
        const syncTaskService = request.diScope.resolve('syncTaskService');
...
const startIncrementalSync = async (request, reply) => {
    try {
        const { xnxq, batchSize, maxRetries, timeout } = request.body;
        request.log.info({ xnxq, batchSize, maxRetries, timeout }, '收到增量同步请求');
        const syncTaskService = request.diScope.resolve('syncTaskService');
 ...
 * 获取任务状态
 */
const getTaskStatus = async (request, reply) => {
    try {
        ...
        request.log.error({ error, taskId: request.params.taskId }, '获取任务状态失败');
        reply.status(500).send({
            success: false,
 ...
 * 获取任务进度
 */
const getTaskProgress = async (request, reply) => {
    try {
        const { taskId } = request.params;
        const syncTaskService = request.diScope.resolve('syncTaskService');
        const progress = await syncTaskService.getTaskProgress(taskId);
        ...
        request.log.error({ error, taskId: request.params.taskId }, '获取任务进度失败');
        reply.status(500).send({
            success: false,
 ...
 * 重试任务
 */
const retryTask = async (request, reply) => {
    try {
        const { taskId } = request.params;
        const { reason } = request.body;
        const syncTaskService = request.diScope.resolve('syncTaskService');
 ...
 * 注册所有同步相关的路由
 */
export async function syncController(fastify) {
    // 启动同步任务
    ...
    fastify.post('/api/sync/incremental', {
        schema: {
            body: {
                type: 'object',
                required: ['xnxq'],
    ...
    // 查询任务状态
    fastify.get('/api/sync/task/:taskId/status', {
        schema: {
            params: {
                type: 'object',
                properties: {
    ...
    fastify.get('/api/sync/task/:taskId/progress', {
        schema: {
            params: {
                type: 'object',
                properties: {
    ...
    // 控制任务
    fastify.post('/api/sync/task/:taskId/pause', {
        schema: {
            params: {
                type: 'object',
                properties: {
    ...
    fastify.post('/api/sync/task/:taskId/resume', {
        schema: {
            params: {
                type: 'object',
                properties: {
    ...
    fastify.post('/api/sync/task/:taskId/cancel', {
        schema: {
            params: {
                type: 'object',
                properties: {
    ...
    fastify.post('/api/sync/task/:taskId/retry', {
        schema: {
            params: {
                type: 'object',
                properties: {
