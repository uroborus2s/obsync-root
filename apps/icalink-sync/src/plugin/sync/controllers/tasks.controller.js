 ...
 * 格式化任务响应数据
 */
const formatTaskToRunning = (task) => {
    ...
    if (isDatabaseEntity) {
        ...
        return {
            ...
            started_at: task.data.startedAt?.toISOString() || null,
            completed_at: task.data.completedAt?.toISOString() || null,
            children: (task.children || [])
                .filter((child) => 'data' in child)
 ...
 * 格式化数据库任务为树形响应格式
 */
const formatDatabaseTaskToTreeResponse = (task, childrenCount) => {
    return {
        id: task.id,
        parent_id: task.parent_id,
        description: task.description,
 ...
 * 获取任务统计信息
 */
const getStatistics = async (request, reply) => {
    try {
        const taskTreeService = request.diScope.resolve('taskTreeService');
 ...
 */
const getTaskTree = async (request, reply) => {
    try {
        const { maxDepth = 1, includePlaceholders = true, limit = 20, offset = 0, status } = request.query;
        // 转换状态参数
        let statusFilter;
        if (status) {
        ...
        const options = {
            ...
            includePlaceholders: includePlaceholders === 'true' || includePlaceholders === true,
            limit: Number(limit),
 ...
 * 获取树形根任务列表（用于前端树形展示）
 */
const getTreeRootTasks = async (request, reply) => {
    try {
        ...
        let query = db
            ...
            .select(['parent_id'])
            .select((eb) => eb.fn.count('id').as('child_count'))
            .where('parent_id', 'is not', null)
            .groupBy('parent_id')
            .as('child_counts'), 'rt.id', 'child_counts.parent_id')
            .select([
            'rt.id',
            'rt.parent_id',
            'rt.name',
        ...
        const formattedTasks = tasks.map((task) => formatDatabaseTaskToTreeResponse(task, Number(task.childrenCount || 0)));
        reply.send({
            success: true,
            data: {
 ...
 * 获取任务的子任务列表
 */
const getTaskChildrenList = async (request, reply) => {
    try {
        const { id } = request.params;
        const { includeChildrenCount = true, status, page = 1, page_size = 100 } = request.query;
        const db = request.diScope.resolve('db');
        // 构建状态过滤条件
        const statusFilter = status
            ? Array.isArray(status)
 ...
 * 获取完整的任务树结构
 */
const getCompleteTaskTree = async (request, reply) => {
    try {
        const { id } = request.params;
        ...
        const statusFilter = status
            ...
            : undefined;
        // 递归获取所有子任务
        const getAllDescendants = async (parentId, currentDepth = 0) => {
            ...
            for (const child of children) {
                const formattedChild = formatDatabaseTaskToTreeResponse(child);
                formattedChild.depth = currentDepth + 1;
                // 递归获取子任务的子任务
                const grandChildren = await getAllDescendants(child.id, currentDepth + 1);
        ...
        // 构建完整的树结构
        const formattedRoot = formatDatabaseTaskToTreeResponse(rootTask);
 ...
 * 获取任务的子任务列表
 */
const getTaskChildren = async (request, reply) => {
    try {
        const { id } = request.params;
        const taskTreeService = request.diScope.resolve('taskTreeService');
        ...
        request.log.error('获取任务子节点失败', { taskId: request.params.id }, error);
        reply.status(500).send({
            success: false,
            error: '获取任务子节点失败',
            message: error instanceof Error ? error.message : String(error)
 ...
 * 获取根任务列表
 */
const getRootTasks = async (request, reply) => {
    try {
        const taskTreeService = request.diScope.resolve('taskTreeService');
        ...
        reply.send({
            success: true,
            data: {
                tasks: paginatedTasks.map((task) => formatTaskResponse(task)),
 ...
 * 根据名称获取任务详情
 */
const getTaskByName = async (request, reply) => {
    try {
        ...
        request.log.error('根据名称获取任务详情失败', { taskName: request.params.name }, error);
        reply.status(500).send({
            success: false,
            error: '获取任务详情失败',
            message: error instanceof Error ? error.message : String(error)
 ...
 * 启动任务
 */
const startTask = async (request, reply) => {
    try {
        const { id } = request.params;
 ...
 * 暂停任务
 */
const pauseTask = async (request, reply) => {
    try {
        const { id } = request.params;
        const { reason } = request.body || {};
        const taskTreeService = request.diScope.resolve('taskTreeService');
 ...
 * 恢复任务
 */
const resumeTask = async (request, reply) => {
    try {
        const { id } = request.params;
        const { reason } = request.body || {};
        const taskTreeService = request.diScope.resolve('taskTreeService');
 ...
 * 取消任务
 */
const cancelTask = async (request, reply) => {
    try {
        const { id } = request.params;
        const { reason } = request.body || {};
        const taskTreeService = request.diScope.resolve('taskTreeService');
 ...
 * 标记任务成功
 */
const markTaskSuccess = async (request, reply) => {
    try {
        const { id } = request.params;
 ...
 * 标记任务失败
 */
const markTaskFail = async (request, reply) => {
    try {
        const { id } = request.params;
 ...
 * 恢复运行中的任务
 */
const recoverTasks = async (request, reply) => {
    try {
        const taskTreeService = request.diScope.resolve('taskTreeService');
 ...
 * 启动增量同步
 */
const startIncrementalSync = async (request, reply) => {
    try {
        const { xnxq, batchSize = 50, parallel = false, maxConcurrency = 5 } = request.body;
        const config = {
            xnxq,
            batchSize,
            parallel,
        ...
        const incrementalSyncService = request.diScope.resolve('incrementalSyncService');
 ...
 * 获取增量同步状态
 */
const getIncrementalSyncStatus = async (request, reply) => {
    try {
        ...
        request.log.error('获取增量同步状态失败', { xnxq: request.params.xnxq }, error);
        reply.status(500).send({
            success: false,
            error: '获取增量同步状态失败',
            message: error instanceof Error ? error.message : String(error)
...
const getTasks = async (request, reply) => {
    try {
        const { status, page = 1, page_size = 50, limit: queryLimit, offset: queryOffset } = request.query;
        ...
        const formattedTasks = tasks.map((task) => formatTaskToRunning(task));
        const total = Number(countResult?.total || 0);
        reply.send({
            success: true,
            data: {
 ...
 * 注册所有任务相关的路由
 */
export async function tasksController(fastify) {
    // 新的通用任务查询接口
