 ...
 * @public
 */
export function formatToRFC3339(rq, sj_f, timeZoneOffset = 480 // 默认为北京时间 +08:00
) {
    ...
    if (timeParts.length < 2 || timeParts.length > 3) {
        throw new TypeError(`无效的时间格式: ${sj_f}，期望格式: HH:mm:ss.SSS 或 HH:mm:ss`);
    }
    ...
    const baseTime = milliseconds === '000'
 ...
 * @public
 */
export function calculateCourseTimeAndStatus(rq, sj_f, sj_t) {
    try {
        const currentTime = new Date();
        ...
        if (rq instanceof Date) {
            ...
            if (rq.includes('/')) {
                ...
                const dateParts = rq.split('/');
                if (dateParts.length !== 3) {
                    throw new TypeError(`无效的日期格式: ${rq}，期望格式: YYYY/MM/DD 或 YYYY-MM-DD`);
                }
                const [year, month, day] = dateParts;
        ...
        const parseTime = (timeStr) => {
            ...
            if (timeParts.length < 2) {
                throw new TypeError(`无效的时间格式: ${timeStr}，期望格式: HH:mm:ss.SSS 或 HH:mm`);
            }
        ...
        if (courseStartTime >= courseEndTime) {
            throw new TypeError(`课程开始时间 ${sj_f} 不能晚于或等于结束时间 ${sj_t}`);
        }
        // 计算课程状态
        let courseStatus;
        ...
        throw new TypeError(`计算课程时间失败: ${error instanceof Error ? error.message : String(error)}`);
    }
}
