...
export class LeaveApprovalRepository extends BaseRepository {
     ...
     * 创建请假审批记录
     */
    async createLeaveApproval(params) {
        try {
            const id = randomUUID();
            const now = new Date();
            this.log.debug({
     ...
     * 批量创建请假审批记录
     */
    async batchCreateLeaveApprovals(applicationId, teachers) {
        try {
            this.log.debug({
                applicationId,
    ...
    async getLeaveApprovalById(id) {
        try {
            ...
            this.log.debug({ id, approverId: result.approver_id }, '获取请假审批记录成功');
            return result;
        }
        catch (error) {
            this.log.error({
     ...
     * 根据申请ID获取所有审批记录
     */
    async getLeaveApprovalsByApplicationId(applicationId) {
        try {
            ...
            const results = await this.db
                ...
                .execute();
            this.log.debug({ applicationId, count: results.length }, '获取审批记录成功');
            return results;
        }
        catch (error) {
            this.log.error({
     ...
     * 根据审批人ID获取待审批记录
     */
    async getPendingApprovalsByApproverId(approverId, limit = 20, offset = 0) {
        try {
            this.log.debug({ approverId, limit, offset }, '根据审批人ID获取待审批记录');
            const results = await this.db
                .selectFrom('icalink_leave_approvals')
                .selectAll()
                ...
                .offset(offset)
                .execute();
            this.log.debug({ approverId, count: results.length }, '获取待审批记录成功');
            return results;
        }
        catch (error) {
            this.log.error({
     ...
     * 更新审批结果
     */
    async updateApprovalResult(id, result, comment, approvalTime) {
        try {
     ...
     * 获取教师审批记录
     */
    async getTeacherApprovals(teacherId) {
        try {
      ...
      `.execute(this.db);
            ...
            this.log.info('查询结果详情:', {
                ...
                sampleData: results.rows.slice(0, 2).map((row) => ({
                    approval_id: row.id,
            ...
            // 为每个申请获取附件信息和课程详细信息
            const applicationsWithAttachments = await Promise.all(results.rows.map(async (app) => {
                // 获取附件信息
                ...
                const result = {
                    ...app,
                    // 构建完整的课程时间
                    class_time: courseDetail?.st && courseDetail?.ed
            ...
            this.log.debug({ teacherId, count: applicationsWithAttachments.length }, '获取教师审批记录成功');
     ...
     * 获取教师审批统计
     */
    async getTeacherApprovalStats(teacherId) {
        try {
     ...
     */
    async processApproval(approvalId, action, comment) {
        try {
            ...
            // 3. 获取该请假申请的所有审批记录
            const allApprovals = await this.getLeaveApprovalsByApplicationId(approval.application_id);
            // 4. 分析所有审批记录的状态
            ...
            if (finalStatus !== 'pending') {
                await this.updateLeaveApplicationStatus(approval.application_id, finalStatus);
            }
            this.log.info({
                approvalId,
     ...
     */
    async updateLeaveApplicationStatus(applicationId, status) {
        try {
            this.log.debug({ applicationId, status }, '更新请假申请状态');
            // 根据最终状态确定学生考勤状态
            const attendanceStatus = status === 'approved' ? 'leave' : 'leave_rejected';
            await this.db
                ...
                .execute();
            this.log.info({ applicationId, status, attendanceStatus }, '请假申请状态更新成功');
        }
        catch (error) {
            this.log.error({
                error: error instanceof Error ? error.message : String(error),
     ...
     * 用于权限验证
     */
    async getApprovalByIdAndApprover(approvalId, approverId) {
        try {
            ...
            this.log.debug({ approvalId, approverId, applicationId: result.application_id }, '获取审批记录成功');
            return result;
        }
        catch (error) {
            this.log.error({
...
//# sourceMappingURL=leave-approval-repository.js.map