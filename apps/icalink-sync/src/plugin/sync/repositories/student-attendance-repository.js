...
export class StudentAttendanceRepository extends BaseRepository {
     ...
     * 创建学生签到记录
     */
    async createStudentAttendance(params) {
        try {
            const id = randomUUID();
            const now = new Date();
            this.log.debug({
     ...
     * 根据考勤记录ID获取所有学生签到记录
     */
    async findByAttendanceRecordId(attendanceRecordId) {
        try {
     ...
     */
    async findByAttendanceRecordIds(attendanceRecordIds) {
        try {
            this.log.debug({ attendanceRecordIds }, '根据考勤记录ID列表获取学生签到记录');
     ...
     * 根据考勤记录ID和学号获取学生签到记录
     */
    async getStudentAttendanceByRecordAndStudent(attendanceRecordId, studentId) {
        try {
            this.log.debug({ attendanceRecordId, studentId }, '根据考勤记录ID和学号获取学生签到记录');
            const result = await this.db
                .selectFrom('icalink_student_attendance')
                .selectAll()
                .where('attendance_record_id', '=', attendanceRecordId)
     ...
     * 根据条件查询学生签到记录
     */
    async findStudentAttendances(conditions, limit, offset) {
        try {
            ...
            if (conditions.attendance_record_id) {
                query = query.where('attendance_record_id', '=', conditions.attendance_record_id);
            }
            if (conditions.xh) {
            ...
            if (conditions.checkin_time_start) {
                query = query.where('checkin_time', '>=', conditions.checkin_time_start);
            }
            ...
            this.log.debug({ conditions, count: results.length }, '查询学生签到记录完成');
            return results;
        }
        catch (error) {
            this.log.error({
     ...
     * 更新学生签到记录
     */
    async updateStudentAttendance(id, updates) {
        try {
     ...
     * 批量创建学生签到记录
     */
    async batchCreateStudentAttendances(attendanceRecordId, students, defaultStatus = AttendanceStatus.ABSENT) {
        try {
            this.log.debug({
                attendanceRecordId,
    ...
    async getStudentAttendanceHistory(studentId, limit = 50, offset = 0) {
        try {
            ...
            const results = await this.db
                ...
                .offset(offset)
                .execute();
            this.log.debug({ studentId, count: results.length }, '获取学生签到历史记录完成');
            return results;
        }
        catch (error) {
            this.log.error({
     ...
     * 检查学生是否已签到
     */
    async isStudentCheckedIn(attendanceRecordId, studentId) {
        try {
            const record = await this.getStudentAttendanceByRecordAndStudent(attendanceRecordId, studentId);
     ...
     * 获取考勤记录的签到学生列表
     */
    async getCheckedInStudents(attendanceRecordId) {
        try {
            ...
            const results = await this.db
                ...
                .execute();
            this.log.debug({ attendanceRecordId, count: results.length }, '获取已签到学生列表完成');
            return results;
        }
        catch (error) {
            this.log.error({
     ...
     */
    async getAttendanceRecordWithStudentDetails(attendanceRecordId) {
        try {
            ...
            // 合并学生信息和签到记录
            const results = courseStudents.map((student) => {
     ...
     * 获取学生请假申请记录
     */
    async getStudentLeaveApplications(studentId, status, startDate, endDate, limit = 20, offset = 0) {
        try {
            this.log.debug({ studentId, status, startDate, endDate, limit, offset }, '查询学生请假审批记录');
            // 获取总数 - 创建单独的计数查询（移除有问题的LEFT JOIN）
            let countQuery = this.db
                .selectFrom('icalink_student_attendance')
                .innerJoin('icalink_attendance_records', 'icalink_student_attendance.attendance_record_id', 'icalink_attendance_records.id')
            ...
            if (status && status !== 'all') {
                countQuery = countQuery.where('icalink_student_attendance.status', '=', status);
            }
            if (startDate) {
                countQuery = countQuery.where('icalink_attendance_records.rq', '>=', startDate.toISOString().split('T')[0]);
            }
            if (endDate) {
                countQuery = countQuery.where('icalink_attendance_records.rq', '<=', endDate.toISOString().split('T')[0]);
            }
            ...
            // 应用相同的筛选条件
            if (status && status !== 'all') {
                dataQuery = dataQuery.where('icalink_student_attendance.status', '=', status);
            }
            if (startDate) {
                dataQuery = dataQuery.where('icalink_attendance_records.rq', '>=', startDate.toISOString().split('T')[0]);
            }
            if (endDate) {
                dataQuery = dataQuery.where('icalink_attendance_records.rq', '<=', endDate.toISOString().split('T')[0]);
            }
            ...
            if (results.length > 0) {
                const courseQueries = results.map((result) => ({
                    kkh: result.kkh,
                    xnxq: result.xnxq,
            ...
            const applications = results.map((result) => {
                // 直接使用数据库中的状态
                ...
                const { courseStartTime, courseEndTime } = calculateCourseTimeAndStatus(classDate, startTime, endTime);
                return {
                    id: result.id,
                    ...
                    application_time: result.application_time?.toISOString() ||
                        result.created_at.toISOString(),
                    status: applicationStatus,
                    approval_time: applicationStatus !== 'leave_pending'
                    ...
                    course_info: {
                        ...
                        room_s: courseInfo?.room_s || classLocationMap.get(result.kkh) || '',
            ...
            this.log.debug({ studentId, total, count: applications.length }, '查询学生请假审批记录完成');
     ...
     * 获取学生请假审批统计信息
     */
    async getStudentLeaveApplicationStats(studentId, startDate, endDate) {
        try {
            this.log.debug({ studentId, startDate, endDate }, '获取学生请假审批统计信息');
            let query = this.db
                .selectFrom('icalink_student_attendance')
                .innerJoin('icalink_attendance_records', 'icalink_student_attendance.attendance_record_id', 'icalink_attendance_records.id')
                .where('icalink_student_attendance.xh', '=', studentId)
            ...
            // 日期范围筛选
            if (startDate) {
                query = query.where('icalink_attendance_records.rq', '>=', startDate.toISOString().split('T')[0]);
            }
            if (endDate) {
                query = query.where('icalink_attendance_records.rq', '<=', endDate.toISOString().split('T')[0]);
            }
     ...
     * 教师查询请假申请记录
     */
    async getTeacherLeaveApplications(teacherId, status, startDate, endDate, limit = 20, offset = 0) {
        try {
            this.log.debug({ teacherId, status, startDate, endDate, limit, offset }, '教师查询请假申请记录');
            // 构建查询条件 - 查询所有有请假原因的记录（表示这是请假申请）
            let query = this.db
                .selectFrom('icalink_student_attendance')
                .innerJoin('icalink_attendance_records', 'icalink_student_attendance.attendance_record_id', 'icalink_attendance_records.id')
                .leftJoin('out_xsxx', 'icalink_student_attendance.xh', 'out_xsxx.xh')
                .leftJoin('juhe_renwu', (join) => join
            ...
            if (status) {
                if (status === 'pending') {
                    query = query.where('icalink_student_attendance.status', '=', 'leave_pending');
                }
            ...
            if (status) {
                if (status === 'pending') {
                    countQuery = countQuery.where('icalink_student_attendance.status', '=', 'leave_pending');
                }
                else if (status === 'processed') {
                    countQuery = countQuery.where('icalink_student_attendance.status', 'in', ['leave', 'absent']);
                }
            }
            if (startDate) {
                countQuery = countQuery.where('icalink_attendance_records.rq', '>=', startDate.toISOString().split('T')[0]);
            ...
            const countResult = await countQuery
                ...
                .executeTakeFirst();
            const total = Number(countResult?.total || 0);
            this.log.info({ teacherId, total, courseCount: courseIds.length }, '查询到的请假申请总数');
            // 如果没有数据，直接返回空结果
            if (total === 0) {
            ...
            if (status) {
                if (status === 'pending') {
                    dataQuery = dataQuery.where('icalink_student_attendance.status', '=', 'leave_pending');
                }
                else if (status === 'processed') {
                    dataQuery = dataQuery.where('icalink_student_attendance.status', 'in', ['leave', 'absent']);
                }
            }
            if (startDate) {
                dataQuery = dataQuery.where('icalink_attendance_records.rq', '>=', startDate.toISOString().split('T')[0]);
            ...
            const applications = results.map((result) => {
                ...
                return {
                    ...
                    application_time: result.application_time?.toISOString() ||
                        result.created_at.toISOString(),
                    status: applicationStatus,
                    approval_time: applicationStatus !== 'pending'
                        ? result.application_time?.toISOString() ||
                            result.created_at.toISOString()
                        : undefined,
                    approval_comment: undefined // 当前数据结构中没有审批意见
            ...
            this.log.debug({ teacherId, total, count: applications.length }, '教师查询请假申请记录完成');
            return { applications, total };
        }
        catch (error) {
            this.log.error({
     ...
     * 获取教师请假申请统计信息
     */
    async getTeacherLeaveApplicationStats(teacherId, startDate, endDate) {
        try {
            this.log.debug({ teacherId, startDate, endDate }, '获取教师请假申请统计信息');
            // 教师筛选条件 - 通过课程关联查找该教师的课程
            const teacherCoursesQuery = this.db
                .selectFrom('u_jw_kcb_cur')
                .select('kkh')
     ...
     * 获取课程历史考勤数据
     */
    async getCourseAttendanceHistory(kkh, xnxq, startDate, endDate) {
        try {
            ...
            // 学年学期筛选
            if (xnxq) {
                attendanceQuery = attendanceQuery.where('icalink_attendance_records.xnxq', '=', xnxq);
            }
            // 日期范围筛选
            if (startDate) {
                attendanceQuery = attendanceQuery.where('icalink_attendance_records.rq', '>=', startDate.toISOString().split('T')[0]);
            }
            if (endDate) {
                attendanceQuery = attendanceQuery.where('icalink_attendance_records.rq', '<=', endDate.toISOString().split('T')[0]);
            }
            // 按日期排序
            ...
            for (const record of attendanceRecords) {
                ...
                // 判断课程实时状态
                const { courseStatus } = calculateCourseTimeAndStatus(record.rq, record.sj_f, record.sj_t);
                // 根据课程状态决定显示数字还是N/A
                const shouldShowNA = courseStatus === 'not_started' || courseStatus === 'in_progress';
                const attendanceRate = stats.total_count > 0 && !shouldShowNA
                    ? Math.round((stats.present_count / stats.total_count) * 100 * 100) / 100
                    : shouldShowNA
                        ? 'N/A'
                        : 0;
                const classStats = {
            ...
            // 计算总体统计
            // 找到第一个已结束的课程来获取总学生数
            const finishedClass = attendanceHistory.find((cls) => cls.course_status === 'finished');
            const overallStats = {
                total_classes: attendanceRecords.length,
                average_attendance_rate: attendanceRecords.length > 0
                    ? Math.round((totalAttendanceRate / attendanceRecords.length) * 100) / 100
                    : 0,
     ...
     * 教师审批请假申请
     */
    async approveLeaveApplication(applicationId, teacherId, action, comment) {
        try {
            this.log.debug({ applicationId, teacherId, action, comment }, '教师审批请假申请');
            // 检查申请是否存在且属于该教师审批
     ...
     * 根据课程号统计每个学生的考勤信息
     */
    async getPersonalCourseStats(kkh, xnxq) {
        try {
            ...
            let attendanceRecordsQuery = this.db
                ...
                .where('kkh', '=', kkh);
            if (xnxq) {
                attendanceRecordsQuery = attendanceRecordsQuery.where('xnxq', '=', xnxq);
            }
            ...
            for (const student of students) {
                ...
                const studentAttendanceRecords = await this.db
                    .selectFrom('icalink_student_attendance')
                    .leftJoin('icalink_attendance_records', 'icalink_student_attendance.attendance_record_id', 'icalink_attendance_records.id')
                    .select([
                    'icalink_student_attendance.status',
                    ...
                    .where('icalink_attendance_records.kkh', '=', kkh)
                    .$if(!!xnxq, (qb) => qb.where('icalink_attendance_records.xnxq', '=', xnxq))
                    .orderBy('icalink_attendance_records.rq', 'desc')
                    .execute();
                // 统计各种状态的次数
                let presentCount = 0;
            ...
            const overallAttendanceRate = totalExpectedAttendance > 0
                ? Math.round((totalPresentCount / totalExpectedAttendance) * 100 * 100) / 100
                : 0;
            // 按出勤率降序排序
     ...
     */
    async withdrawStudentLeaveApplication(studentId, attendanceRecordId) {
        try {
    ...
    async findWithDetails(params) {
        const { studentId, week, startTime, endTime, page = 1, pageSize = 10 } = params;
        let baseQuery = this.db
            .selectFrom('icalink_student_attendance as sa')
            .innerJoin('icalink_attendance_records as a', 'a.id', 'sa.attendance_record_id');
        if (studentId) {
...
//# sourceMappingURL=student-attendance-repository.js.map