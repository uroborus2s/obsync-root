...
export class LeaveAttachmentRepository extends BaseRepository {
   ...
   * 创建请假附件
   */
  async createLeaveAttachment(
    params: CreateLeaveAttachmentParams
   ...
   * 根据ID获取请假附件
   */
  async getLeaveAttachmentById(
    id: string
  ): Promise<LeaveAttachmentEntity | null> {
    try {
      ...
      return result as LeaveAttachmentEntity;
    } catch (error) {
      this.log.error(
        {
          error: error instanceof Error ? error.message : String(error),
   ...
   * 根据申请ID获取所有附件
   */
  async getLeaveAttachmentsByApplicationId(
    ...
    try {
      ...
      return results as LeaveAttachmentEntity[];
    } catch (error) {
      this.log.error(
        {
          error: error instanceof Error ? error.message : String(error),
   ...
   * 删除请假附件
   */
  async deleteLeaveAttachment(id: string): Promise<boolean> {
    try {
   ...
   * 批量创建请假附件
   */
  async batchCreateLeaveAttachments(
    attachments: CreateLeaveAttachmentParams[]
  ): Promise<LeaveAttachmentEntity[]> {
    try {
      ...
      const records = attachments.map((attachment) => ({
        ...
        storage_type: attachment.storage_type || ('database' as const),
        upload_time: now
      }));
      ...
      const createdRecords = records.map((record) => ({
        ...
        storage_type: record.storage_type as 'file' | 'database'
      })) as LeaveAttachmentEntity[];
