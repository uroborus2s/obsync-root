...
export class UserCalendarRepository {
     ...
     * 如果记录已存在则更新，不存在则创建
     */
    async associateCalendar(params) {
        try {
            this.log.debug({ xgh: params.xgh, calendarId: params.calendar_id }, '关联日历ID和学号');
            // 先检查是否已存在记录
            ...
            if (existingRecord) {
                ...
                this.log.info({ xgh: params.xgh, calendarId: params.calendar_id }, '日历ID关联更新成功');
                ...
                this.log.info({ xgh: params.xgh, calendarId: params.calendar_id }, '日历ID关联创建成功');
                return newRecord;
            }
        }
        catch (error) {
     ...
     * 批量关联日历ID和学号
     */
    async batchAssociateCalendar(associationList) {
        try {
    ...
    async dissociateCalendar(xgh) {
        try {
            ...
            const result = await this.db
                ...
                .executeTakeFirst();
            const updated = Number(result.numUpdatedRows) > 0;
            this.log.info({ xgh, updated }, updated ? '日历关联取消成功' : '未找到相关记录');
            return updated;
        }
        catch (error) {
            this.log.error({
     ...
     */
    async findXghByCalendarId(calendarId) {
        try {
