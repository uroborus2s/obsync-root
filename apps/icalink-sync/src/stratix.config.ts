...
import { loadEnvironment, type SensitiveConfig } from './config/environment.js';
 ...
 */
function createStratixConfig(sensitiveInfo: SensitiveConfig): StratixConfig {
  ...
  return {
    ...
    logger: {
      level: sensitiveInfo.logger.loglevle as
        | 'trace'
        | 'debug'
        | 'info'
        | 'warn'
 ...
 * @returns Stratix 配置对象
 */
export default function createConfigWithEnvironment(
  sensitiveInfo?: SensitiveConfig
): StratixConfig {
  try {
    // 如果没有提供敏感配置，则从环境变量加载
