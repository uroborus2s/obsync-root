// 环境配置加载工具
// 负责从环境变量中加载和解密敏感配置信息
import { validateConfig as coreValidateConfig, decryptConfig, encryptConfig, loadConfigFromFile } from '@stratix/core';
...
export class EnvironmentLoader {
     ...
     * @returns 加密后的字符串
     */
    static encryptConfig(data) {
        try {
            return encryptConfig(data);
        }
        catch (error) {
            throw new Error(`配置加密失败: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     * 解密配置信息
    ...
    static decryptConfig(encryptedData) {
        try {
            ...
            throw new Error(`配置解密失败: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     * 从文件加载配置
    ...
    static loadFromFile(filePath) {
        try {
            ...
            throw new Error(`从文件加载配置失败: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     * 验证配置对象的完整性
