...
import { type UserAuthModule } from '@stratix/was-v7';
 ...
 * @returns 处理后的URL
 */
function addUrlParams(baseUrl: string, params: Record<string, string>): string {
  try {
...
const handleAuthCode =
  ...
  async (
    ...
    try {
      const userAuthService = request.diScope.resolve<UserAuthModule>(
        'wasV7UserAuth'
      ) as UserAuthModule;
      ...
      if (request.query.state) {
        try {
          callbackUrl = Buffer.from(request.query.state, 'base64').toString(
            'utf8'
          );
        } catch (e) {
