...
# 默认配置
DEFAULT_PROJECT_NAME="icalink-api"
...
# 显示帮助信息
show_help() {
    echo -e "${BLUE}iCalink API Docker 构建和推送脚本${NC}"
    echo ""
    echo "用法:"
    ...
    echo "示例:"
    echo "  $0                                    # 使用默认参数"
    echo "  $0 icalink-api v1.0.0                # 指定项目和版本"
    echo "  $0 --no-cache icalink-api v1.0.0     # 禁用缓存构建"
    echo "  $0 --dry-run icalink-api v1.0.0      # 预览命令"
    echo ""
}
...
check_environment() {
    ...
    if [ ! -f "package.json" ] || [ ! -f "dockerfile" ] || [ ! -f ".npmrc" ]; then
        log_error "请在 icalink-api 项目根目录下运行此脚本"
...
main() {
    ...
    echo -e "${BLUE}🚀 iCalink API Docker 构建和推送${NC}"
    echo "======================================"
    ...
    if [ "$DRY_RUN" = false ]; then
        ...
        echo "  docker run -d -p 8090:8090 --name ${PROJECT_NAME} ${FULL_IMAGE_NAME}"
    fi
}
