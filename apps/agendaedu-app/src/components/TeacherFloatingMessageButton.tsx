...
export function TeacherFloatingMessageButton({
  className = ''
}: TeacherFloatingMessageButtonProps) {
  ...
  return (
    <div className={`fixed bottom-6 right-4 z-50 ${className}`}>
      <button
        onClick={handleClick}
        disabled={isLoading}
        className='relative flex h-14 w-14 items-center justify-center rounded-full bg-green-500 text-white shadow-lg transition-all duration-300 hover:scale-110 hover:bg-green-600 active:scale-95 disabled:opacity-50'
        aria-label={
          pendingCount > 0
