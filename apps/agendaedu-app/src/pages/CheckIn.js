...
import { AlertCircle, ArrowLeft, Calendar, CheckCircle, ChevronDown, ChevronUp, Clock, MapPin, RefreshCw, Share2, TrendingUp, User } from 'lucide-react';
...
export function CheckIn() {
    ...
    const [location, setLocation] = useState(FIXED_LOCATION.address || '');
    const [locationInfo, setLocationInfo] = useState(FIXED_LOCATION);
    ...
    const [wpsAuthStatus, setWpsAuthStatus] = useState({ isAuthorized: false });
    useEffect(() => {
    ...
    const loadAttendanceData = async () => {
        ...
        setError(null);
        try {
            const response = await attendanceApi.getStudentAttendanceRecord(attendanceId);
    ...
    const loadCourseHistory = async () => {
        if (!attendanceData?.data?.course?.kkh)
            return;
    ...
    const getSemesterStartDate = (xnxq) => {
    ...
    // 计算周数
    const getWeekNumber = (classDate, semesterStart) => {
    ...
    // 格式化课程时间显示
    const formatClassTime = (classItem, semesterStart) => {
        // 优先使用教学周信息，如果没有则计算周数
        const weekNum = classItem.teaching_week ||
    ...
    const handleGetLocation = async () => {
        ...
        try {
            if (wpsAuthService.isWPSEnvironment() && wpsAuthStatus.isAuthorized) {
                ...
                setLocation(wpsLocation.address ||
    ...
    const handleCheckIn = async () => {
        ...
        setIsLoading(true);
        try {
            let checkInLocationResult = null;
            ...
            if (wpsAuthService.isWPSEnvironment() && wpsAuthStatus.isAuthorized) {
                ...
                try {
                    // 执行完整的打卡流程（位置验证 + 可选拍照）
                    checkInLocationResult = await wpsAuthService.performCheckIn(targetLocation, 100, // 最大允许距离100米
                    false // 不强制要求拍照
                    );
                    if (!checkInLocationResult.isValidLocation) {
                        await wpsAuthService.showToast(`距离目标位置${checkInLocationResult.distance}米，超出允许范围`, 'error');
                        return;
                    }
                    console.log('✅ 位置验证通过，距离:', checkInLocationResult.distance, '米');
                }
                catch (error) {
            ...
            // 使用验证后的位置或默认位置进行签到
            const finalLocationInfo = checkInLocationResult?.location || locationInfo || FIXED_LOCATION;
            const checkInRequest = {
                location: finalLocationInfo.address || '默认位置',
                latitude: finalLocationInfo.latitude,
                longitude: finalLocationInfo.longitude
            };
            const response = await attendanceApi.studentCheckIn(attendanceId, checkInRequest);
            if (response.success) {
                setIsCheckedIn(true);
                if (wpsAuthService.isWPSEnvironment() && wpsAuthStatus.isAuthorized) {
                ...
                if (wpsAuthService.isWPSEnvironment() && wpsAuthStatus.isAuthorized) {
                    await wpsAuthService.showToast(response.message || '签到失败，请重试', 'error');
                }
                else {
    ...
    if (error && !attendanceData) {
        ...
        <div className='flex items-center justify-center py-8'>
          <div className='rounded-lg bg-white p-8 text-center shadow-sm'>
            ...
            <button onClick={loadAttendanceData} className='rounded-lg bg-blue-500 px-4 py-2 text-white hover:bg-blue-600'>
              重试
            </button>
          </div>
        </div>
    ...
    return (<div className='min-h-screen bg-gray-50'>
      ...
      <div className='bg-white shadow-sm'>
        <div className='flex items-center justify-between px-4 py-4'>
          ...
          <button onClick={loadAttendanceData} disabled={isLoadingData} className='rounded-lg p-2 hover:bg-gray-100 disabled:opacity-50' aria-label='刷新'>
      ...
      <div className='space-y-6 p-4'>
        {/* 错误提示 */}
        {error && (<div className='rounded-lg bg-red-50 p-4'>
            <div className='flex items-center'>
        ...
        {/* 课程信息 */}
        {attendanceData?.data && (<div className='rounded-lg bg-white p-4 shadow-sm'>
            <div className='mb-4 flex items-center justify-between'>
              <h2 className='text-lg font-semibold text-gray-900'>课程信息</h2>
              <span className={`rounded-full px-2 py-1 text-xs ${getAttendanceStatusColor(attendanceData.data.attendance_status.status)}`}>
                {getAttendanceStatusText(attendanceData.data.attendance_status.status)}
              </span>
            </div>
            ...
            <div className='space-y-3'>
              ...
              {attendanceData.data.course.room_s && (<div className='flex items-center'>
              ...
              {attendanceData.data.course.xm_s && (<div className='flex items-center'>
              ...
              {attendanceData.data.course.jxz && (<div className='flex items-center'>
              ...
              {attendanceData.data.course.lq && (<div className='flex items-center'>
                  <MapPin className='mr-3 h-4 w-4 text-gray-400'/>
                  <span className='text-gray-600'>
              ...
              <div className='flex items-center'>
                ...
                <span className={`text-sm font-medium ${attendanceData.data.course.status === 'not_started'
                ? 'text-gray-600'
        ...
        {/* 签到统计 */}
        {attendanceData?.data?.stats && (<div className='rounded-lg bg-white p-4 shadow-sm'>
        ...
        <div className='rounded-lg bg-white shadow-sm'>
          <button onClick={toggleHistory} className='flex w-full items-center justify-between p-4 text-left hover:bg-gray-50'>
            <div className='flex items-center'>
          ...
          {showHistory && (<div className='border-t border-gray-200 p-4'>
              {isLoadingHistory ? (<div className='py-4 text-center'>
                ...
                </div>) : historyData?.data ? (<div className='space-y-3'>
                  {/* 课程总体统计 */}
                  <div className='mb-4 rounded-lg bg-gray-50 p-3'>
                    ...
                    <div className='grid grid-cols-2 gap-4 text-sm md:grid-cols-4'>
                      ...
                      <div>
                        ...
                        <span className={`font-medium ${getAttendanceRateColor(historyData.data.overall_stats.average_attendance_rate)}`}>
                          {historyData.data.overall_stats.average_attendance_rate.toFixed(1)}
                          %
                        </span>
                      </div>
                      <div>
                  ...
                  <div className='space-y-2'>
                    ...
                    const semesterStart = getSemesterStartDate(historyData.data.course_info.xnxq);
                    return (<div key={classItem.attendance_record_id} className='flex items-center justify-between rounded-lg border border-gray-200 p-3 hover:bg-gray-50'>
                          <div className='flex-1'>
                            <div className='font-medium text-gray-900'>
                          ...
                          <div className='text-right'>
                            <div className={`text-sm font-medium ${getAttendanceRateColor(classItem.attendance_rate)}`}>
        ...
        <div className='rounded-lg bg-white p-4 shadow-sm'>
          <div className='mb-4 flex items-center justify-between'>
            ...
            <button onClick={handleGetLocation} disabled={isLoading} className='rounded-lg bg-blue-500 px-3 py-1 text-sm text-white hover:bg-blue-600 disabled:opacity-50'>
              {isLoading ? '获取中...' : '获取位置'}
            </button>
          </div>
        ...
        {/* 签到按钮 */}
        {!isCheckedIn &&
            !attendanceData?.data?.attendance_status.is_checked_in ? (<button onClick={handleCheckIn} disabled={isLoading} className='w-full rounded-lg bg-green-500 py-4 text-lg font-medium text-white hover:bg-green-600 disabled:opacity-50'>
          ...
          </button>) : (<div className='rounded-lg bg-green-50 p-4 text-center'>
            ...
            <button onClick={handleShare} className='mt-3 flex items-center justify-center rounded-lg bg-blue-500 px-4 py-2 text-sm text-white hover:bg-blue-600'>
