...
import { AlertCircle, ArrowLeft, Calendar, Clock, FileText, MapPin, RefreshCw, Upload, User, X } from 'lucide-react';
...
function LeaveContent() {
        ...
        try {
            ...
            if (attachments.length > 0) {
                ...
                try {
                    ...
                    toast.error('文件处理失败', {
                        description: fileError instanceof Error ? fileError.message : '文件读取失败',
                        duration: 4000
                    });
            ...
            if (response.success) {
                ...
                setTimeout(() => {
                    navigate(`/attendance/student?id=${encodeURIComponent(attendanceId)}`);
                }, 1500);
            }
            else {
    ...
    const handleFileUpload = (event) => {
        ...
        if (files) {
            ...
            const newFilesTotalSize = candidateFiles.reduce((sum, file) => sum + file.size, 0);
            if (currentTotalSize + newFilesTotalSize > maxTotalSize) {
                const totalSizeMB = ((currentTotalSize + newFilesTotalSize) /
                    (1024 * 1024)).toFixed(2);
                toast.error('文件总大小超限', {
    ...
    const canSubmitLeave = () => {
        return (attendanceData?.data?.attendance_status.can_leave &&
    ...
    return (<div className='min-h-screen bg-gray-50'>
      ...
      <div className='bg-white shadow-sm'>
        <div className='flex items-center justify-between px-4 py-4'>
          ...
          <button onClick={loadAttendanceData} disabled={isLoading} className='rounded-lg p-2 hover:bg-gray-100 disabled:opacity-50' aria-label='刷新'>
        ...
        {attendanceData?.data && (<div className='rounded-lg bg-white p-4 shadow-sm'>
            ...
            <div className='space-y-3'>
              ...
              <div className='flex items-center text-gray-600'>
                ...
                new Date(attendanceData.data.course.course_start_time).toLocaleDateString('zh-CN')}
                </span>
              </div>
              ...
              <div className='flex items-center text-gray-600'>
                ...
                new Date(attendanceData.data.course.course_start_time).toLocaleTimeString('zh-CN', {
                    hour: '2-digit',
                    minute: '2-digit'
                })}{' '}
                  -{' '}
                  {attendanceData.data.course.sj_t ||
                new Date(attendanceData.data.course.course_end_time).toLocaleTimeString('zh-CN', {
                    hour: '2-digit',
                    minute: '2-digit'
                })}
                </span>
        ...
        <div className='rounded-lg bg-white p-4 shadow-sm'>
          ...
          <div className='mb-4'>
            ...
            <div className='grid grid-cols-2 gap-2'>
              {['sick', 'personal', 'emergency', 'other'].map((type) => (<button key={type} onClick={() => setLeaveType(type)} className={`rounded-lg border px-3 py-2 text-sm transition-colors ${leaveType === type
          ...
          <div className='mb-4'>
            ...
            <textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder='请详细说明请假原因...' className='w-full rounded-lg border border-gray-200 p-3 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500' rows={4} maxLength={500}/>
          ...
          <div className='mb-6'>
            ...
            <div className='space-y-2'>
              {/* 上传按钮 */}
              {attachments.length < 3 && (<label className='flex cursor-pointer items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-4 hover:border-gray-400'>
                  <div className='text-center'>
              ...
              {/* 已上传的附件列表 */}
              {attachments.map((file, index) => (<div key={index} className='flex items-center justify-between rounded-lg border border-gray-200 p-3'>
                  ...
                  <button onClick={() => removeAttachment(index)} className='rounded-lg p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-600' title='移除图片' aria-label='移除图片'>
                    <X className='h-4 w-4'/>
                  </button>
                </div>))}
            </div>
          ...
          {/* 提交按钮 */}
          <div className='space-y-3'>
            {!canSubmitLeave() && (<div className='rounded-lg bg-yellow-50 p-3'>
                <div className='flex items-center'>
            ...
            <button onClick={handleSubmit} disabled={isSubmitting || !canSubmitLeave()} className={`w-full rounded-lg px-4 py-3 font-semibold transition-colors ${isSubmitting || !canSubmitLeave()
