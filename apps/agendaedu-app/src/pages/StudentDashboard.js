...
function StudentDashboardContent() {
    ...
    const loadAttendanceData = async () => {
        ...
        setError(null);
        try {
            const response = await api.get(`/attendance/${encodeURIComponent(id)}/record?type=student`);
        ...
        <div className='mx-auto max-w-md space-y-4 p-4'>
          ...
          <div className='rounded-lg bg-white p-6 shadow-sm'>
            <div className='mb-4 flex items-center justify-between'>
              ...
              <div className={`rounded-full px-3 py-1 text-sm font-medium ${course.status === 'not_started'
            ...
            <div className='space-y-3'>
              <div className='flex items-center text-gray-600'>
                ...
                <span className='text-sm'>
                  {new Date(course.course_start_time).toLocaleDateString('zh-CN', {
                month: '2-digit',
                day: '2-digit',
                weekday: 'short'
            })}
              ...
              <div className='flex items-center text-gray-600'>
                ...
                <span className='text-sm'>
                  {new Date(course.course_start_time).toLocaleTimeString('zh-CN', {
                hour: '2-digit',
                minute: '2-digit'
            })}{' '}
                  -{' '}
          ...
          <div className='rounded-lg bg-white p-6 shadow-sm'>
            <div className='mb-6 text-center'>
              {attendance_status.is_checked_in ? (<div className='text-green-600'>
                  <div className='mb-3 text-4xl'>✓</div>
                  <div className='text-xl font-semibold'>已签到</div>
                  {attendance_status.checkin_time && (<div className='mt-2 text-sm text-gray-500'>
                      签到时间：
                      {new Date(attendance_status.checkin_time).toLocaleString()}
                    </div>)}
                </div>) : attendance_status.status === 'leave_pending' ? (<div className='text-yellow-600'>
                  <div className='mb-3 text-4xl'>⏳</div>
                  <div className='text-xl font-semibold'>请假审批中</div>
                  <div className='mt-2 text-sm text-gray-500'>
                    请在右下角消息处查看审核状态或撤回请假重新签到
                  </div>
                </div>) : attendance_status.status === 'leave' ? (<div className='text-orange-600'>
                  <div className='mb-3 text-4xl'>📝</div>
            ...
            {!attendance_status.is_checked_in &&
                ...
                attendance_status.status !== 'leave_pending' && (<div className='space-y-3'>
                  {/* 签到按钮 */}
                  <button onClick={handleCheckin} disabled={checkinLoading || !attendance_status.can_checkin} className={`w-full rounded-lg px-4 py-3 font-semibold transition-colors ${!attendance_status.can_checkin
                  ...
                  {/* 请假按钮 */}
                  <button onClick={() => navigate(`/leave/${encodeURIComponent(id)}`)} disabled={!attendance_status.can_leave} className={`w-full rounded-lg px-4 py-3 font-semibold transition-colors ${!attendance_status.can_leave
                    ? 'cursor-not-allowed bg-gray-300 text-gray-500'
            ...
            {/* 状态提示信息 */}
            {attendance_status.is_checked_in && (<div className='py-2 text-center text-gray-500'>您已完成签到</div>)}
          </div>
          ...
          <div className='rounded-lg bg-gray-50 p-4 text-sm text-gray-700'>
            <p>
              <strong>请假时间：</strong>
              {new Date(attendance_status.auto_start_time).toLocaleString()}{' '}
              之前
            </p>
          </div>
        </div>
