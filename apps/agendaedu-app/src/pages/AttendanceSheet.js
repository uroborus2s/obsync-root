...
import { wpsCollaboration } from '@/lib/wps-collaboration-api';
import { AlertCircle, BarChart3, Calendar, CheckCircle, ChevronDown, MapPin, TrendingUp, User, XCircle } from 'lucide-react';
...
export function AttendanceSheet() {
    ...
    const [courseHistoryData, setCourseHistoryData] = useState(null);
    const [personalStats, setPersonalStats] = useState([]);
    const [courseStats, setCourseStats] = useState(null);
    const [personalCourseStats, setPersonalCourseStats] = useState(null);
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState(null);
    const id = searchParams.get('id');
    // 解码ID获取开课号的函数
    const extractKkhFromId = (encodedId) => {
        try {
    ...
    // 格式化教学周和节次信息
    const formatPeriod = (jc_s, startTime) => {
        const start = new Date(startTime);
        const dateStr = start
            .toLocaleDateString('zh-CN', {
            year: 'numeric',
    ...
    // 格式化历史记录时间显示
    const formatHistoryTime = (classDate, classTime, weekNumber) => {
    ...
    const loadTeacherAttendanceData = async () => {
        ...
        setError(null);
        try {
            const response = await api.get(`/attendance/${encodeURIComponent(id)}/record?type=teacher`);
            ...
            if ((responseData.success && responseData.data) ||
                ...
                throw new Error(responseData.message || response.message || '获取课程信息失败');
            }
        }
        catch (error) {
    ...
    const calculateTotalLeaveCount = (stats, studentDetails) => {
        // 从stats中获取已批准的请假数量
        let totalLeave = stats.leave_count;
        // 如果有学生详情，统计待审批的请假数量
        if (studentDetails) {
            const pendingLeaveCount = studentDetails.filter((student) => student.status === 'leave_pending').length;
            totalLeave += pendingLeaveCount;
        }
        return totalLeave;
    };
    // 兼容原有签到表功能的状态处理函数
    const getLegacyStatusIcon = (status) => {
        switch (status) {
            case 'present':
                return <CheckCircle className='h-4 w-4 text-green-500'/>;
            case 'late':
    ...
    const getLegacyStatusText = (status) => {
        switch (status) {
            case 'present':
                return '已签到';
            case 'late':
    ...
    const getLegacyStatusColor = (status) => {
        switch (status) {
            case 'present':
                return 'text-green-600 bg-green-50';
            case 'late':
    ...
    if (id && teacherData) {
        ...
        return (<div className='flex h-screen flex-col bg-gray-50'>
        <div className='mx-auto flex max-w-4xl flex-1 flex-col bg-white shadow-lg'>
          {/* 头部标签页 */}
          <div className='bg-white shadow-sm'>
            <div className='flex border-b'>
              <button onClick={() => setActiveTab('current')} className={`flex-1 px-4 py-3 text-center font-medium transition-colors ${activeTab === 'current'
                ? 'border-b-2 border-blue-600 bg-blue-50 text-blue-600'
                : 'text-gray-600 hover:text-gray-800'}`}>
                本节课签到
              </button>
              <button onClick={() => setActiveTab('history')} className={`flex-1 px-4 py-3 text-center font-medium transition-colors ${activeTab === 'history'
                ? 'border-b-2 border-blue-600 bg-blue-50 text-blue-600'
                : 'text-gray-600 hover:text-gray-800'}`}>
                历史统计
              </button>
              <button onClick={() => setActiveTab('personal')} className={`flex-1 px-4 py-3 text-center font-medium transition-colors ${activeTab === 'personal'
                ? 'border-b-2 border-blue-600 bg-blue-50 text-blue-600'
                : 'text-gray-600 hover:text-gray-800'}`}>
                更多
              </button>
          ...
          <div className='flex-1 overflow-hidden p-4'>
            {activeTab === 'current' && (<>
                ...
                <div className='mb-6 rounded-lg bg-blue-50 p-4'>
                  ...
                  <div className='mb-3 text-lg font-medium text-blue-600'>
                    {formatCourseTime(course_status.course_start_time, course_status.course_end_time)}
                  </div>
                  <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
                    ...
                    <div className='space-y-2 text-sm text-gray-600'>
                      <p>
                        ...
                        {formatPeriod(course.jc_s, course_status.course_start_time)}
                      </p>
                      <p>
                      ...
                      <p>
                        ...
                        <span className={`ml-1 rounded px-2 py-1 text-xs ${getCourseStatusColor(course_status.status)}`}>
                ...
                <div className='mb-6 rounded-lg bg-white p-4 shadow-sm'>
                  <div className='grid grid-cols-4 gap-4 text-center'>
                    ...
                    <div className='rounded-lg bg-blue-50 p-4'>
                      <div className='text-3xl font-bold text-blue-600'>
                        {calculateTotalLeaveCount(stats, teacherData.student_details)}
                      </div>
                ...
                {teacherData.student_details &&
                    teacherData.student_details.length > 0 && (<div className='mb-6 rounded-lg bg-white p-4 shadow-sm'>
                      <h3 className='mb-4 font-semibold text-gray-800'>
                        学生签到情况
                      </h3>
                      <div className='max-h-96 space-y-2 overflow-y-auto'>
                        {teacherData.student_details.map((student, index) => (<div key={index} className='flex items-center justify-between rounded-lg border border-gray-200 p-3 hover:bg-gray-50'>
                            ...
                            student.checkin_time && (<span className='text-xs text-gray-500'>
                                    {new Date(student.checkin_time).toLocaleTimeString('zh-CN', {
                                hour: '2-digit',
                                minute: '2-digit'
                            })}
                                  </span>)}
                              ...
                              {/* 状态标签 */}
                              <span className={`flex items-center space-x-1 rounded-full px-2 py-1 text-xs ${getStatusColor(student.status)}`}>
            ...
            {activeTab === 'history' && (<div className='flex h-full flex-col'>
                {/* 如果有课程历史数据，显示真实数据 */}
                {courseHistoryData?.success && courseHistoryData.data ? (<>
                      ...
                      <div className='grid grid-cols-4 gap-4 text-center'>
                        ...
                        <div className='rounded-lg bg-green-50 p-4'>
                          <div className='text-2xl font-bold text-green-600'>
                            {courseHistoryData.data.overall_stats.average_attendance_rate.toFixed(1)}
                            %
                          </div>
                    ...
                    <div className='flex-1 rounded-lg bg-white p-4 shadow-sm'>
                      ...
                      <div className='flex h-full flex-col'>
                        ...
                        .map((record) => (<div key={record.attendance_record_id} className='rounded-lg border border-gray-200 p-4'>
                                <div className='mb-2 flex items-center justify-between'>
                                  <div className='flex items-center space-x-2'>
                                    <h4 className='font-medium text-gray-900'>
                                      {record.teaching_week}周
                                    </h4>
                                    <span className={`rounded-full px-2 py-1 text-xs ${getCourseStatusColor(record.course_status)}`}>
                                      {getCourseStatusText(record.course_status)}
                                    </span>
                                  </div>
                                  <div className='text-sm text-gray-500'>
                                    {formatHistoryTime(record.class_date, record.class_time)}
                                  </div>
                                </div>
            ...
            {activeTab === 'personal' && (<div className='flex h-full flex-col'>
                {/* 如果有个人课程统计数据，显示真实数据 */}
                {personalCourseStats?.success && personalCourseStats?.data ? (<>
                    ...
                    <div className='mb-4 rounded-lg bg-white p-4 shadow-sm'>
                      ...
                      <div className='mb-6 grid grid-cols-3 gap-4 text-center'>
                        ...
                        <div className='rounded-lg bg-orange-50 p-4'>
                          <div className='text-2xl font-bold text-orange-600'>
                            {personalCourseStats.data.course_info.overall_attendance_rate.toFixed(1)}
                            %
                          </div>
                          <div className='text-sm text-gray-600'>
                            整体出勤率
                    ...
                    <div className='flex flex-1 flex-col rounded-lg bg-white p-4 shadow-sm'>
                      ...
                      <div className='flex-1 space-y-4 overflow-y-auto'>
                        {personalCourseStats.data.student_stats.map((student, index) => (<div key={index} className='rounded-lg border border-gray-200 p-4'>
                              {/* 学生基本信息 */}
                              <div className='mb-3 flex items-center justify-between'>
                                <div className='flex items-center space-x-3'>
                                  ...
                                  <div>
                                    ...
                                    {student.zymc && (<div className='text-xs text-gray-400'>
                                        {student.zymc}
                                      </div>)}
                                  </div>
                                </div>
                            ...
                            student.recent_records.length > 0 && (<div>
                                    ...
                                    <div className='space-y-1'>
                                      {student.recent_records
                                .slice(0, 3)
                                .map((record, recordIndex) => (<div key={recordIndex} className='flex items-center justify-between text-xs'>
                                            <span className='text-gray-600'>
                                              {new Date(record.class_date).toLocaleDateString('zh-CN')}
                                            </span>
                                            <span className={`rounded-full px-2 py-1 ${record.status === 'present'
    ...
    return (<div className='min-h-screen bg-gray-50'>
      ...
      <div className='bg-white shadow-sm'>
        ...
        <div className='flex border-b'>
          ...
          <div className='relative'>
            <button onClick={() => setShowDropdown(!showDropdown)} className={`flex items-center space-x-1 px-4 py-3 font-medium transition-colors ${activeTab === 'personal'
            ? 'border-b-2 border-blue-600 bg-blue-50 text-blue-600'
            : 'text-gray-600 hover:text-gray-800'}`}>
            ...
            {showDropdown && (<div className='absolute right-0 top-full z-10 w-32 rounded-lg border border-gray-200 bg-white shadow-lg'>
                <button onClick={() => {
      ...
      <div className='p-4'>
        {activeTab === 'current' && currentData && (<div className='space-y-4'>
            {/* 课程信息卡片 */}
            <div className='rounded-lg bg-white p-4 shadow-sm'>
              ...
              <div className='space-y-2'>
                ...
                <div className='max-h-64 space-y-2 overflow-y-auto'>
                  {currentData.records.map((record, index) => (<div key={index} className='flex items-center justify-between rounded-lg border border-gray-200 p-3'>
                      <div className='flex items-center space-x-3'>
                        <div className='flex h-8 w-8 items-center justify-center rounded-full bg-gray-100'>
                          <User className='h-4 w-4 text-gray-600'/>
                        </div>
                      ...
                      <div className='flex items-center space-x-2'>
                        {record.checkInTime && (<span className='text-xs text-gray-500'>
                            {record.checkInTime}
                          </span>)}
                        <span className={`flex items-center space-x-1 rounded-full px-2 py-1 text-xs ${getLegacyStatusColor(record.status)}`}>
            ...
            {courseHistoryData?.success && courseHistoryData.data ? (<>
                ...
                <div className='rounded-lg bg-white p-4 shadow-sm'>
                  ...
                  <div className='space-y-3'>
                    ...
                    .map((record, index) => (<div key={record.attendance_record_id} className='rounded-lg border border-gray-200 p-4'>
                            <div className='mb-2 flex items-center justify-between'>
                              <h4 className='font-medium text-gray-900'>
                                第{index + 1}节课
                              </h4>
                              <div className='text-sm text-gray-500'>
                                {formatHistoryTime(record.class_date, record.class_time, record.teaching_week)}
                              </div>
                            </div>
            ...
            <div className='rounded-lg bg-white p-4 shadow-sm'>
                ...
                <div className='space-y-3'>
                  {historyData
                    ...
                    .map((data, index) => (<div key={index} className='rounded-lg border border-gray-200 p-4'>
                        <div className='mb-2 flex items-center justify-between'>
        ...
        {activeTab === 'personal' && (<div className='space-y-4'>
            ...
            <div className='rounded-lg bg-white p-4 shadow-sm'>
              ...
              {/* 统计数据 */}
              {personalStats.map((stat, index) => (<div key={index} className='mb-4 last:mb-0'>
                  <div className='mb-2 flex items-center justify-between'>
                  ...
                  <div className='mb-2 h-2 rounded-full bg-gray-200'>
                    <div className='h-2 rounded-full bg-blue-500' style={{ width: `${stat.attendanceRate}%` }}></div>
                  </div>
            ...
            {/* 课程统计概览 */}
            {courseStats && (<div className='rounded-lg bg-white p-4 shadow-sm'>
                <div className='mb-4 flex items-center space-x-2'>
