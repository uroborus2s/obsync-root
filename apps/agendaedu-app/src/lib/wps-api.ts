...
class WPSAPIService {
  ...
  async initialize(config: WPSConfig): Promise<void> {
    try {
      if (typeof window !== 'undefined' && (window as any).WebOfficeSDK) {
  ...
  async createLeaveApplication(data: LeaveApplicationData): Promise<void> {
    if (!this.isReady()) {
...
审批时间：________________`;
  ...
  async createAttendanceSheet(data: AttendanceData): Promise<void> {
    if (!this.isReady()) {
