...
import type {
  ...
  StudentAttendanceSearchResponse,
  StudentInfo
...
export class AttendanceService {
  ...
  async createAttendanceRecord(
    ...
    return await this.attendanceRepo.createAttendanceRecord(
   ...
   * @returns 课程签到信息
   */
  async getCourseAttendanceInfo(
    taskId: string,
    studentId?: string
  ): Promise<CourseAttendanceInfo> {
    // 获取考勤记录
    const attendanceRecord =
      await this.attendanceRepo.getAttendanceRecord(taskId);
    if (!attendanceRecord) {
    ...
    // 检查是否过期
    if (
      attendanceRecord.auto_close_time &&
    ...
    let studentAttendance: StudentAttendanceRecord | undefined;
    ...
    if (studentId) {
      ...
      const studentAttendanceRecord =
        await this.studentAttendanceRepo.getStudentAttendanceByRecordAndStudent(
          taskId,
          studentId
        );
      ...
      if (studentAttendanceRecord) {
        studentAttendance = {
          ...
          status: studentAttendanceRecord.status as AttendanceStatus,
  ...
  async getAttendanceStats(
    ...
    try {
      ...
      const stats =
        await this.studentAttendanceRepo.getAttendanceStats(attendanceRecordId);

      const checkinRate =
        attendanceRecord.total_count > 0
      ...
      return {
        ...
        data: {
          ...
          attendance_status: {
            ...
            status: studentAttendance?.status as AttendanceStatus | undefined,
