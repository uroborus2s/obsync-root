...
- 2026-09-01 `BUG-009` 修复了 `@stratix/was-v7` 的 KSO-1 查询参数签名。`1.0.0-beta.37` 已发布到 Aliyun，但消费端发现它误带入仅适配 Core 1.1.2 的子路径导入，因此不得用于当前应用。用户确认 `1.0.0-beta.38` 只兼容 `@stratix/core@1.0.0-beta.9`；Core 根入口与 Utils async 入口已恢复，Core beta.9 下 123/123 测试、lint、build 及隔离 tarball consumer 加载通过，等待发布和反查。
