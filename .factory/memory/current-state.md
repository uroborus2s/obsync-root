...
- 2026-09-02 `BUG-009` 已关闭：`@stratix/was-v7@1.0.0-beta.38` 发布到 Aliyun，精确版本和 `latest` 反查均为 beta.38，发布元数据中的 Core peer 固定为 `1.0.0-beta.9`。该版本保留 KSO-1 查询参数签名修复，恢复 Core 根入口与 Utils async 入口；Core beta.9 下 123/123 测试、lint、build 及隔离 tarball consumer 加载通过。beta.37 因误带入 Core 1.1.2 子路径不得用于当前应用。
