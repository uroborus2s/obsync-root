...
2026-06-19 `@stratix/core` 闭门复评结论为 83/100，初始口径只能按 RC / controlled release 推进。P0/P1/P2/P2+ 修复后，本地 supported release scope 生产评分达到 `95/100`。该历史分数不替代当前发布准入；2026-06-26 远端 Quality Gate 曾失败，本地修复 `.env.example.tpl` 跟踪问题后 run `28234054546` 已通过，后续仍必须完成 exact tags 和 npm publish 凭证操作。
...
- P2+：provider 插拔化和 plugin `provides` 深校验已完成，本地 supported release scope 达到 `95/100`。远端 Quality Gate run `28234054546` 已通过；95% 全局覆盖率和 npm publish 不属于本地代码修复完成项，其中 npm 仍是发布前外部证据。
...
- 完成 P2+ provider 插拔化和 plugin `provides` 深校验后：本地生产级评分达到 `95/100`。该评分不等于 core 全包覆盖率 95%，也不替代 exact tags 和 npm publish 凭证操作。
