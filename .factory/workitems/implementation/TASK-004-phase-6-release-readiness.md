...
- 实际执行 workspace release-surface gate 时会要求 supported package 存在 exact git tag，且 tag 指向当前 release commit。
...
- `stratix release gate --scope workspace` 已真实执行 build/test/docs/pack/API，并能在 exact tags 缺失或未指向 release commit 时精确阻断于 release-surface。
