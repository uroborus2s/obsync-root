...
- `@stratix/tasks` 决策为 1.1.x 永久冻结/废弃包：保留源码作历史参考，排除 supported build/test/release gate 和新项目推荐路径；恢复需单独迁移项目。
- 旧 app/plugin 模板目录物理删除因缺少明确人工批准而延后；forge 源码和发布包继续不读取、不发布这些 create-only 旧目录。
