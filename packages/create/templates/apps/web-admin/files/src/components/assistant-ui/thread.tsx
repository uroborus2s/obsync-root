  ...
  return (
    ...
    >
      ...
      >
        ...
        <ThreadPrimitive.ViewportFooter className='aui-thread-viewport-footer max-w-(--thread-max-width) rounded-t-(--composer-radius) bg-background sticky bottom-0 mx-auto mt-auto flex w-full flex-col gap-4 overflow-visible pb-4 md:pb-6'>
  ...
  return (
    <div className='aui-thread-welcome-suggestion-display fade-in slide-in-from-bottom-2 @md:nth-[n+3]:block nth-[n+3]:hidden animate-in fill-mode-both duration-200'>
      <SuggestionPrimitive.Trigger send asChild>
        <Button
          variant='ghost'
          className='aui-thread-welcome-suggestion @md:flex-col bg-background hover:bg-muted h-auto w-full flex-wrap items-start justify-start gap-1 rounded-3xl border px-4 py-3 text-left text-sm transition-colors'
        >
    ...
    <ComposerPrimitive.Root className='aui-composer-root relative flex w-full flex-col'>
      <ComposerPrimitive.AttachmentDropzone asChild>
        <div
          ...
          className='rounded-(--composer-radius) bg-background p-(--composer-padding) focus-within:border-ring/75 focus-within:ring-ring/20 data-[dragging=true]:border-ring data-[dragging=true]:bg-accent/50 flex w-full flex-col gap-2 border transition-shadow focus-within:ring-2 data-[dragging=true]:border-dashed'
        >
          <ComposerAttachments />
          <ComposerPrimitive.Input
            placeholder='Send a message...'
            className='aui-composer-input px-1.75 placeholder:text-muted-foreground/80 max-h-32 min-h-10 w-full resize-none bg-transparent py-1 text-sm outline-none'
            rows={1}
            autoFocus
...
const AssistantMessage: FC = () => {
  return (
    <MessagePrimitive.Root
      ...
      data-role='assistant'
    >
      <div className='aui-assistant-message-content wrap-break-word text-foreground px-2 leading-relaxed'>
  ...
  return (
    ...
    >
      ...
      <div className='aui-user-message-content-wrapper relative col-start-2 min-w-0'>
        <div className='aui-user-message-content wrap-break-word bg-muted text-foreground peer rounded-2xl px-4 py-2.5 empty:hidden'>
          <MessagePrimitive.Parts />
        </div>
        <div className='aui-user-action-bar-wrapper absolute left-0 top-1/2 -translate-x-full -translate-y-1/2 pr-2 peer-empty:hidden'>
          <UserActionBar />
        </div>
      </div>
