 ...
 * 插件默认配置
 */
export const DEFAULT_PLUGIN_CONFIG: Partial<DatabasePluginOptions> = {
  // 默认连接名称
...
export class PluginConfigValidator {
   ...
   * 验证插件配置
   */
  static validateConfig(options: any): DatabasePluginOptions {
    try {
      // 基础配置验证
      ...
      for (const [name, config] of Object.entries(options.connections)) {
        this.validateConnectionConfig(name, config as any);
      }

      // 验证默认连接存在
      const defaultConnection =
        options.defaultConnection || DEFAULT_PLUGIN_CONFIG.defaultConnection;
      ...
      // 合并默认配置
      const validatedConfig = deepMerge(
        DEFAULT_PLUGIN_CONFIG,
        options
      ) as DatabasePluginOptions;
   ...
   * 验证单个连接配置
   */
  private static validateConnectionConfig(name: string, config: any): void {
    if (!config || typeof config !== 'object') {
      throw new ConfigurationError(
        `Connection '${name}' configuration must be an object`
      );
    }
    ...
    if (config.type === 'sqlite') {
      if (!config.database) {
        throw new ConfigurationError(
          `SQLite connection '${name}' must specify a database file path`
        );
      }
    } else {
      // 其他数据库类型的验证
      if (!config.host) {
        throw new ConfigurationError(
          `Connection '${name}' must specify a host`
        );
      }
   ...
   * 验证连接池配置
   */
  private static validatePoolConfig(
    connectionName: string,
    poolConfig: any
  ): void {
    if (typeof poolConfig !== 'object') {
      throw new ConfigurationError(
        `Connection '${connectionName}' pool configuration must be an object`
      );
    }
    ...
    for (const field of numericFields) {
      if (poolConfig[field] !== undefined) {
        if (typeof poolConfig[field] !== 'number' || poolConfig[field] < 0) {
          throw new ConfigurationError(
            `Connection '${connectionName}' pool.${field} must be a non-negative number`
          );
        }
      }
    }
    ...
    if (poolConfig.min !== undefined && poolConfig.max !== undefined) {
      if (poolConfig.min > poolConfig.max) {
        throw new ConfigurationError(
          `Connection '${connectionName}' pool.min (${poolConfig.min}) cannot be greater than pool.max (${poolConfig.max})`
        );
      }
    }
  }
   ...
   * 验证健康检查配置
   */
  static validateHealthCheckConfig(config: any): void {
    if (!config) return;

    if (typeof config !== 'object') {
      throw new ConfigurationError(
        'Health check configuration must be an object'
      );
    }
    ...
    for (const field of numericFields) {
      if (config[field] !== undefined) {
        if (typeof config[field] !== 'number' || config[field] <= 0) {
          throw new ConfigurationError(
            `Health check ${field} must be a positive number`
          );
        }
      }
    }
   ...
   * 验证监控配置
   */
  static validateMonitoringConfig(config: any): void {
    if (!config) return;

    if (typeof config !== 'object') {
      throw new ConfigurationError(
        'Monitoring configuration must be an object'
      );
    }
    ...
    for (const field of numericFields) {
      if (config[field] !== undefined) {
        if (typeof config[field] !== 'number' || config[field] <= 0) {
          throw new ConfigurationError(
            `Monitoring ${field} must be a positive number`
          );
        }
      }
    }
 ...
 * 配置工厂函数
 */
export const createPluginConfig = (options: any): DatabasePluginOptions => {
 ...
 * 获取自动发现配置
 */
export const getAutoDiscoveryConfig = (overrides: any = {}) => {
...
export const ConfigHelpers = {
   ...
   * 检查是否启用健康检查
   */
  isHealthCheckEnabled: (config: DatabasePluginOptions): boolean => {
   ...
   * 获取健康检查端点
   */
  getHealthCheckEndpoint: (config: DatabasePluginOptions): string => {
    return (
   ...
   * 检查是否启用监控
   */
  isMonitoringEnabled: (config: DatabasePluginOptions): boolean => {
