...
export interface IDatabaseManager {
   ...
   * 处理数据库错误并执行恢复逻辑
   */
  handleDatabaseError(error: Error, context?: unknown): Promise<void>;
   ...
   * 检查所有连接健康状态
   */
  checkAllHealth(): Promise<DatabaseResult<HealthStatus[]>>;
   ...
   * 获取预创建状态
   */
  getPreCreationStatus(): PreCreationStatus;
...
export interface IDatabaseDialect {
   ...
   * 检查驱动依赖是否可用
   */
  checkDriverAvailability(): Promise<DatabaseResult<boolean>>;

  /**
   * 处理连接错误
   */
  handleConnectionError(error: Error, connectionName?: string): Error;
