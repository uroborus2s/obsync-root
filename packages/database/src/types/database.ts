...
import type { DatabaseResult } from '../utils/helpers.js';
import type { 
  DatabaseType, 
  ConnectionStatus, 
  HealthStatus, 
...
import type { ConnectionConfig, DatabaseConfig } from './configuration.js';
...
export interface IQueryLogger {
   ...
   * 记录查询
   */
  logQuery(query: string, params: any[], duration: number, connection: string): void;
  
  /**
   * 记录慢查询
   */
  logSlowQuery(query: string, params: any[], duration: number, connection: string): void;
  
  /**
   * 记录查询错误
   */
  logQueryError(query: string, params: any[], error: Error, connection: string): void;
