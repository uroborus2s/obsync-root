...
describe('并发事务支持测试', () => {
  ...
  describe('AsyncLocalStorage 并发隔离', () => {
    it('多个并发事务应该完全隔离', async () => {
      ...
      // 验证事务对象正确对应
      expect(
        results.find((r) => r.currentTransaction === transaction1)
      ).toBeDefined();
      expect(
        results.find((r) => r.currentTransaction === transaction2)
      ).toBeDefined();
      expect(
        results.find((r) => r.currentTransaction === transaction3)
      ).toBeDefined();
    });
  ...
  describe('高并发场景模拟', () => {
    ...
    it('并发事务中的错误应该独立处理', async () => {
      ...
      const promises = [
        // 第一个事务抛出错误
        transactionContextManager
          .runInTransaction(transaction1, async () => {
            await new Promise((resolve) => setTimeout(resolve, 5));
  ...
  describe('真实场景模拟', () => {
    it('模拟多个用户同时进行转账操作', async () => {
      ...
      const promises = transferOperations.map((transfer, index) => {
        ...
        return transactionContextManager.runInTransaction(
          transaction,
          async () => {
            // 模拟转账操作的多个步骤
