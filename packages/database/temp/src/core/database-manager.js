...
import { eitherLeft, eitherRight, isLeft, isRight } from '@stratix/utils/functional';
...
class DatabaseManager {
    ...
    constructor(config, connectionFactory) {
        ...
        this.connectionCreationPromises = new Map();
            ...
            error: error
                ? error instanceof Error
                    ...
                    : {
                        ...
                        code: error.code,
                        connectionName: 'connectionName' in error ? error.connectionName : undefined,
    ...
    async validateEnvironment() {
        if (this.debugEnabled) {
            console.log('🔍 EnhancedDatabaseManager: Starting environment validation...');
        }
        try {
            // 1. 验证数据库配置
            ...
            if (this.debugEnabled) {
                console.log('✅ EnhancedDatabaseManager: Environment validation completed');
            }
        }
        catch (error) {
     ...
     */
    async initializeConnectionFactory() {
    ...
    async initializeConnection() {
        if (this.debugEnabled) {
            console.log('🚀 EnhancedDatabaseManager: Starting connection pre-creation...');
        }
        ...
        try {
            ...
            if (!result.success) {
                throw new Error(`Failed to pre-create connections: ${result.error?.message}`);
            }
            ...
            if (this.debugEnabled) {
                console.log(`✅ EnhancedDatabaseManager: ${this.connections.size} connections pre-created in ${this.preCreationStatus.duration}ms`);
            }
            // 启动连接健康监控
            await this.startHealthMonitoring();
        }
        catch (error) {
            this.preCreationStatus.errors.push(error instanceof Error ? error.message : String(error));
            console.error('❌ EnhancedDatabaseManager: Connection pre-creation failed:', error);
            throw error; // 阻止应用启动
        }
    }
    /**
    ...
    async onClose() {
        if (this.debugEnabled) {
            console.log('🔄 EnhancedDatabaseManager: Starting graceful connection shutdown...');
        }
        try {
            // 停止健康监控
            ...
            if (this.debugEnabled) {
                console.log(`✅ EnhancedDatabaseManager: ${successCount}/${results.length} connections closed successfully`);
            }
        }
        catch (error) {
            console.error('❌ EnhancedDatabaseManager: Error during connection shutdown:', error);
        }
    }
    /**
    ...
    async handleDatabaseError(error, context) {
        if (this.debugEnabled) {
            console.error('💥 EnhancedDatabaseManager: Database error occurred:', error);
        }
        try {
            // 记录错误统计
            ...
            if (this.isConnectionError(error)) {
                if (this.debugEnabled) {
                    console.log('🔄 EnhancedDatabaseManager: Attempting connection recovery...');
                }
                // 执行连接恢复
                const recoveryResult = await this.attemptConnectionRecovery(error);
                if (recoveryResult.success) {
                    if (this.debugEnabled) {
                        console.log('✅ EnhancedDatabaseManager: Connection recovery successful');
                    }
                }
                else {
                    console.error('❌ EnhancedDatabaseManager: Connection recovery failed:', recoveryResult.error);
                }
            }
            // 检查是否需要触发断路器
            ...
            console.error('❌ EnhancedDatabaseManager: Error recovery failed:', recoveryError);
        }
    }
    /**
     * 验证数据库配置
    ...
    async validateDatabaseConfig() {
        ...
        if (!this.config.connections[defaultConnectionName]) {
            throw new Error(`Default database connection '${defaultConnectionName}' not configured`);
        }
        // 验证所有连接配置
        ...
        for (const [name, config] of Object.entries(allConnections)) {
            if (!config.type || !config.database) {
                throw new Error(`Invalid configuration for connection '${name}': missing type or database`);
            }
        }
    }
    /**
    ...
    async checkDriverAvailability() {
        ...
        for (const driverType of requiredDrivers) {
            try {
                ...
                throw new Error(`Database driver '${driverType}' is not available: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
    /**
     * 验证连接配置
     */
    async validateConnectionConfigs() {
        const allConnections = this.getAllConnections();
        for (const [name, config] of Object.entries(allConnections)) {
            if (!config.type) {
    ...
    async checkSystemResources() {
        ...
        const freeMemoryMB = Math.round((memoryUsage.heapTotal - memoryUsage.heapUsed) / 1024 / 1024);
        if (freeMemoryMB < 50) {
            // 至少需要50MB可用内存
            console.warn(`⚠️ Low memory available: ${freeMemoryMB}MB`);
        }
        if (this.debugEnabled) {
            console.log(`📊 Memory usage: ${Math.round(memoryUsage.heapUsed / 1024 / 1024)}MB used, ${freeMemoryMB}MB free`);
        }
    }
    // 移除了不存在的配置方法，简化实现
    /**
    ...
    async preCreateAllConnections() {
        try {
            ...
            if (this.debugEnabled) {
                console.log(`🔧 Pre-creating ${configs.length} database connections...`);
            }
            // 并行创建所有连接
            const connectionPromises = configs.map(async ({ name, config }) => {
                const startTime = Date.now();
                try {
                    const connectionResult = await this.connectionFactory.createConnection(config);
            ...
            if (failures.length > 0) {
                const errorMessages = failures.map((f) => f.status === 'fulfilled' ? f.value.error : 'Unknown error');
                return DatabaseErrorHandler.failure(new ConnectionError(`Failed to create ${failures.length} connections: ${errorMessages.join('; ')}`));
            }
            return DatabaseErrorHandler.success(undefined);
        }
        catch (error) {
            return DatabaseErrorHandler.failure(new ConnectionError(`Connection pre-creation failed: ${error instanceof Error ? error.message : String(error)}`));
        }
    }
    /**
     * 启动连接健康监控
     ...
     * 记录错误统计
     */
    async recordErrorMetrics(error, context) {
        const errorInfo = {
     ...
     * 检查错误是否与特定连接相关
     */
    isConnectionRelatedError(error, connectionName) {
        return (error.message.includes(connectionName) ||
     ...
     * 尝试连接恢复
     */
    async attemptConnectionRecovery(_error) {
        if (this.recoveryStatus.inProgress) {
            return DatabaseErrorHandler.failure(new ConnectionError('Connection recovery already in progress'));
        }
        ...
        try {
            if (this.recoveryStatus.attemptCount > this.recoveryStatus.maxAttempts) {
                return DatabaseErrorHandler.failure(new ConnectionError(`Max recovery attempts (${this.recoveryStatus.maxAttempts}) exceeded`));
            }
            // 等待退避时间
            if (this.recoveryStatus.attemptCount > 1) {
                const backoffTime = this.recoveryStatus.backoffMs *
                    Math.pow(2, this.recoveryStatus.attemptCount - 1);
                if (this.debugEnabled) {
                    console.log(`⏳ Waiting ${backoffTime}ms before recovery attempt ${this.recoveryStatus.attemptCount}`);
                }
            ...
            if (this.debugEnabled) {
                console.log(`🔄 Attempting to recover ${failedConnections.length} failed connections`);
            }
                ...
                try {
                    ...
                    if (oldConnection) {
                        try {
                            ...
                            console.warn(`Warning: Failed to close old connection ${name}:`, closeError);
                        }
                    }
                    // 更新连接
                    this.connections.set(name, connectionResult.data);
                    ...
                    console.error(`❌ Failed to recover connection ${name}:`, recoveryError);
                    return {
                        name,
                        success: false,
            ...
            const successfulRecoveries = recoveryResults.filter((r) => r.status === 'fulfilled' && r.value.success).length;
            this.recoveryStatus.inProgress = false;
            if (successfulRecoveries > 0) {
                // 重置恢复计数器，因为至少有一些连接恢复成功
                this.recoveryStatus.attemptCount = 0;
                if (this.debugEnabled) {
                    console.log(`✅ Successfully recovered ${successfulRecoveries}/${failedConnections.length} connections`);
                }
                return DatabaseErrorHandler.success(undefined);
            }
            else {
                return DatabaseErrorHandler.failure(new ConnectionError(`Failed to recover any of the ${failedConnections.length} failed connections`));
            }
        }
        catch (error) {
            this.recoveryStatus.inProgress = false;
            return DatabaseErrorHandler.failure(new ConnectionError(`Connection recovery failed: ${error instanceof Error ? error.message : String(error)}`));
        }
    }
    /**
     * 检查断路器状态
     */
    async checkCircuitBreaker(_error) {
        // 简化断路器实现
        const errorRate = this.calculateErrorRate();
        const threshold = 0.5; // 默认阈值
        if (errorRate > threshold) {
            this.log(`Circuit breaker threshold exceeded: ${errorRate} > ${threshold}`);
            // 可以在这里实现断路器打开逻辑
        }
    }
    /**
     ...
     * 验证连接配置是否存在
     */
    validateConnectionConfig(connectionName) {
        const allConnections = this.getAllConnections();
     ...
     * 安全地从缓存获取连接
     */
    getConnectionFromCache(connectionName) {
        const connection = this.connections.get(connectionName);
        if (!connection) {
            return eitherLeft(new ConnectionError(`Connection '${connectionName}' not found in cache`));
        }
     ...
     * 线程安全地创建新连接
     */
    async createConnectionSafely(connectionName, config) {
        // 检查是否已有创建中的Promise
        ...
        if (existingPromise) {
            try {
                ...
                return eitherLeft(new ConnectionError(`Failed to wait for existing connection creation: ${error instanceof Error ? error.message : String(error)}`));
            }
        }
        // 创建新的连接Promise
        ...
        try {
            ...
            this.logError(`Connection creation failed`, error instanceof Error ? error : new Error(String(error)), {
                connectionName,
                connectionType: config.type,
                operation: 'createConnectionSafely'
            });
     ...
     * 创建新连接的实际实现
     */
    async createNewConnection(connectionName, config) {
        const startTime = Date.now();
        ...
        const duration = Date.now() - startTime;
        if (!connectionResult.success) {
            this.logError(`Connection factory failed to create connection`, connectionResult.error instanceof Error
            ...
            throw (connectionResult.error || new Error('Unknown connection creation error'));
        }
     ...
     * 更新连接使用统计
     */
    updateConnectionStats(connectionName) {
     ...
     * 获取连接（现在支持自动创建）
     */
    async getConnection(connectionName = 'default') {
        const startTime = Date.now();
        ...
        if (isLeft(configResult)) {
            this.logError(`Connection configuration validation failed`, configResult.left, {
                connectionName,
                operation: 'getConnection'
            });
        ...
        const creationResult = await this.createConnectionSafely(connectionName, configResult.right);
     ...
     * 获取读连接（支持读写分离）
     */
    async getReadConnection(connectionName = 'default') {
        // 尝试获取专用的读连接
     ...
     * 获取写连接（支持读写分离）
     */
    async getWriteConnection(connectionName = 'default') {
        // 尝试获取专用的写连接
    ...
    async checkAllHealth() {
        try {
            const healthPromises = Array.from(this.connections.entries()).map(async ([name, connection]) => {
                const startTime = Date.now();
                try {
                    // 执行简单的健康检查查询
                    await connection
            ...
            const healthStatuses = healthResults.map((result) => result.status === 'fulfilled'
                ? result.value
                : {
                    healthy: false,
                    responseTime: 0,
            ...
            return DatabaseErrorHandler.failure(new ConnectionError(`Health check failed: ${error instanceof Error ? error.message : String(error)}`));
        }
    }
}
_a = RESOLVER;
