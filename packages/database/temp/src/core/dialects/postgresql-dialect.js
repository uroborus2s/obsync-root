    ...
    validateConfig(config) {
        ...
        try {
            ...
            if (!config.connectionString) {
                ...
                if (!config.database) {
                    throw new Error('Database name is required for PostgreSQL connections');
                }
            }
            // 验证SSL配置
            if (config.ssl && config.ssl.mode) {
                ...
                if (!validModes.includes(config.ssl.mode)) {
                    throw new Error(`Invalid SSL mode for PostgreSQL: ${config.ssl.mode}`);
                }
            }
 ...
 * 创建PostgreSQL方言实例
 */
export const createPostgreSQLDialect = () => {
