...
export class SQLiteDialect extends BaseDialect {
    constructor() {
        super(...arguments);
        this.type = 'sqlite';
        this.defaultPort = 0; // SQLite不使用端口
    }
    /**
     * 创建 Kysely 实例
     */
    async createKysely(config, logger) {
    ...
    validateConfig(config) {
        ...
        try {
            ...
            const dbPath = this.getDatabasePath(config);
            if (!dbPath) {
                throw new Error('Database path is required for SQLite connections. Use database field or sqlite.filename.');
            }
            // 验证路径格式
     ...
     * 确保目录存在
     */
    async ensureDirectoryExists(dbPath) {
        if (dbPath === ':memory:') {
            return;
        }
    ...
    createDatabaseOptions(config) {
        ...
        if (config.options?.verbose) {
            options.verbose = (message) => {
                try {
                    // 尝试从框架获取日志器
                    const { getLogger: getFrameworkLogger } = require('@stratix/core/logger');
     ...
     * 设置Pragma选项
     */
    setPragmaOptions(database, config) {
        // 默认pragma设置
 ...
 * 创建SQLite方言实例
 */
export const createSQLiteDialect = () => {
