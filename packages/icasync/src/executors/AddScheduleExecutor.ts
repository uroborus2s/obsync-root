...
export default class AddScheduleExecutor implements TaskExecutor {
  ...
  async execute(context: ExecutionContext): Promise<ExecutionResult> {
    ...
    try {
      ...
      const calendar_id = mappingResult.data.calendar_id;
      ...
      const result: CreateSchedulesResult = {
        ...
        calendar_id,
        schedule_count: schedules.length,
        success_count: createResult.data!.length,
        batch_index,
        created_schedules: createResult.data!,
        duration: Date.now() - startTime
      };
  ...
  private async updateStatusAfterScheduleCreation(
    ...
    try {
      ...
      if (juheUpdateResult.success) {
        juheRenwuUpdatedCount = juheUpdateResult.data;
      ...
      if (directUpdateResult.success) {
        courseRawUpdatedCount = directUpdateResult.data;
