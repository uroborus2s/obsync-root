    ...
    try {
      ...
      if (isRight(updateResult)) {
        this.logger.debug('原始记录状态更新成功', {
          juheRenwuId,
          updatedCount: updateResult.right
        });
        return updateResult.right > 0;
      } else {
    ...
    try {
      ...
      if (!config.dryRun) {
        ...
        if (!createCalendarResult || !createCalendarResult.id) {
          return left('调用WPS API创建日历失败：未返回有效的日历ID'
          );
        }
        ...
        if (!juheRenwuResult.success || !juheRenwuResult.right) {
          return {
            success: false,
        ...
        const xnxq = juheRenwuResult.right.xnxq || 'unknown';
        ...
        if (isLeft(mappingResult)) {
          this.logger.error('创建日历映射失败，需要手动清理日历', {
            calendarId,
            kkh: config.kkh,
            error: mappingResult.left
          });
          return {
            success: false,
