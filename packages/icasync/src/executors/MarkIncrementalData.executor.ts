...
export default class MarkIncrementalDataProcessor implements TaskExecutor {
  ...
  private async performMarkIncrementalData(
    ...
    try {
      ...
      // if (isLeft(statsResult)) {
      //   throw new Error(`获取增量数据统计失败: ${statsResult.error}`);
      // }

      // const incrementalDataStats = statsResult.right;
      ...
      if (!config.dryRun) {
        ...
        if (isLeft(markResult)) {
          throw new Error(`标记juhe_renwu记录失败: ${markResult.error}`);
        }

        markedJuheRenwu = markResult.right.updatedJuheRenwu;
        this.logger.info('juhe_renwu记录标记完成', {
          xnxq: config.xnxq,
          updatedJuheRenwu: markResult.right.updatedJuheRenwu,
          updatedCourseRaw: markResult.right.updatedCourseRaw
        });
      } else {
        // 测试运行：估算将要标记的记录数
        // markedJuheRenwu =
      ...
      if (config.validateResults && !config.dryRun) {
        ...
        if (isRight(validateResult)) {
          validationResult = validateResult.right;
