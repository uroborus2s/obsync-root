  ...
  implements TaskExecutor
{
  ...
  async execute(context: ExecutionContext): Promise<ExecutionResult> {
    ...
    try {
      ...
      if (isLeft(result)) {
        return left(result.error?.message || 'Failed to fetch unprocessed records'
        );
      }
  ...
  private async processAttendanceCourses(
    ...
    try {
      ...
      if (isRight(result)) {
        const createdCount = result.right!.createdCount || 0;
        const skippedCount = result.right!.skippedCount || 0;
        ...
        this.logger.error('创建签到课程记录失败', { error: result.left });
        return 0;
      }
    } catch (error) {
