  ...
  > {
    try {
      ...
      if (isRight(updateResult)) {
        this.logger.info(
          `原始数据状态更新成功, kkh: ${kkh}, studentUpdateCount: ${updateResult.right.studentUpdateCount}, teacherUpdateCount: ${updateResult.right.teacherUpdateCount}, totalUpdateCount: ${updateResult.right.totalUpdateCount}`
        );

        return {
          studentUpdateCount: updateResult.right.studentUpdateCount,
          teacherUpdateCount: updateResult.right.teacherUpdateCount,
          totalUpdateCount: updateResult.right.totalUpdateCount
        };
      } else {
        this.logger.error(
