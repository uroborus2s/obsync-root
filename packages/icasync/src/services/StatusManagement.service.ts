...
import { isLeft, isRight } from '@stratix/utils/functional';
  ...
  implements IStatusManagementService
{
  ...
  async updateSemesterTaskStatus(
    ...
    try {
      ...
      if (isLeft(tasksResult) || !tasksResult.right) {
        const errorMessage = isRight(tasksResult)
          ? 'No data returned'
          : 'Query failed';
        throw new Error(`获取学期任务失败: ${errorMessage}`);
      }

      const tasks = tasksResult.right;
        ...
        if (taskIds.length > 0) {
          try {
            ...
            if (isRight(updateResult)) {
              ...
              result.leftCount += taskIds.length;
              result.lefts.push(
                `批量更新任务状态失败: ${updateResult.left || 'Unknown error'}`
              );
            }
          } catch (error) {
            result.leftCount += taskIds.length;
            ...
            result.lefts.push(`批量更新任务状态异常: ${errorMsg}`);
          }
        }
      }
      ...
      this.logger.info(`学期任务状态更新完成`, {
        ...
        errorCount: result.leftCount
      });

      return result;
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
      this.logger.left(`更新学期任务状态失败: ${errorMsg}`);
      result.leftCount++;
      result.lefts.push(errorMsg);
      return result;
    }
  }
  ...
  async updateCourseSyncStatus(
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      this.logger.left(`更新课程同步状态失败: ${errorMsg}`);
      result.leftCount++;
      result.lefts.push(errorMsg);
      return result;
    }
  ...
  async updateTaskStatusBatch(
    ...
    try {
      ...
      if (isRight(updateResult)) {
        ...
        result.leftCount = taskIds.length;
        result.lefts.push(`批量更新失败: ${updateResult.left?.message}`);
      }

      return result;
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
      this.logger.left(`批量更新任务状态失败: ${errorMsg}`);
      result.leftCount = taskIds.length;
      result.lefts.push(errorMsg);
      return result;
    }
  ...
  async getSyncStatusStats(xnxq: string): Promise<{
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      this.logger.left(`获取同步状态统计失败: ${errorMsg}`);
      return {
        totalTasks: 0,
        completedTasks: 0,
        failedTasks: 0,
