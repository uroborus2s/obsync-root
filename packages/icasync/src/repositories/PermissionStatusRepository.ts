  ...
  implements IPermissionStatusRepository
{
  ...
  async updateStudentCourseStatus(
    ...
    try {
      ...
      this.logger.error('更新学生课程关联状态失败', {
        kkh: config.kkh,
        xnxq: config.xnxq,
        gxZt: config.gxZt,
        error: errorMessage
  ...
  async updateTeacherCourseStatus(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      this.logger.error('更新教师课程关联状态失败', {
        kkh: config.kkh,
        xnxq: config.xnxq,
        gxZt: config.gxZt,
        error: errorMessage
  ...
  async updatePermissionStatus(
    ...
    try {
      ...
      if (studentResult.success) {
        studentUpdateCount = studentResult.data;
      } else {
        errors.push(`学生表更新失败: ${studentResult.error?.message}`);
      }

      if (teacherResult.success) {
        teacherUpdateCount = teacherResult.data;
      } else {
        errors.push(`教师表更新失败: ${teacherResult.error?.message}`);
      }
