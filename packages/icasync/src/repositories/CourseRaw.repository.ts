  ...
  implements ICourseRawRepository
{
  ...
  async findByKkhAndXnxq(
    ...
    try {
      ...
      return left(QueryError.create(errorMessage)
      );
    }
  }
  ...
  async getDistinctChangedCourses(): Promise<DatabaseResult<CourseChange[]>> {
    try {
      ...
      if (isLeft(changesResult)) {
        return left((changesResult as any).error || new Error('Failed to fetch changes')
        );
      }
      ...
      for (const course of changesResult.right) {
      ...
      return right(Array.from(courseChanges.values())
      );
    } catch (error) {
  ...
  async updateGxZtByKkhAndXnxq(
    ...
    try {
      ...
      const result = await this.rightbaseApi.executeQuery(operation, {
  ...
  async findCoursesByTimeSlot(
    ...
    try {
      ...
      return await this.rightbaseApi.executeQuery(operation);
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
    ...
    try {
      ...
      if (isRight(result)) {
        this.logOperation('执行全量聚合查询完成', {
          xnxq,
          aggregatedCount: result.right.length
        });
        return result;
      } else {
  ...
  async executeTransactionalAggregation(
    ...
    try {
      ...
      return await this.rightbaseApi.transaction(operation);
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      this.logError(
    ...
    try {
      ...
      if (isRight(result)) {
        this.logOperation('findOriginalCoursesByJuheRenwuId完成', {
          juheRenwuId,
          foundCount: result.right.length
        });
      }
  ...
  async findOriginalCoursesByJuheRenwuIds(
    ...
    try {
      ...
      if (isRight(result)) {
        ...
        return right(result.right as CourseRaw[]
        );
      } else {
        return left(result.left
        );
  ...
  > {
    try {
      ...
      this.logOperation('updateJuheRenwuByIncrementalData完成', {
        xnxq,
        updatedJuheRenwu: result.right.updatedJuheRenwu,
        updatedCourseRaw: result.right.updatedCourseRaw
      });
