...
/**
 * 批量修复 Executor 文件中的 DatabaseResult Either 类型使用
 */
...
function fixFile(filePath) {
  ...
  if (content.includes('DatabaseResult') && !content.includes("from '@stratix/utils/functional'")) {
    // 找到合适的导入位置（在 @stratix/core 或 @stratix/database 之后）
    const importMatch = content.match(/(import.*from '@stratix\/(core|database)';)/);
    if (importMatch) {
...
// 主程序
console.log('开始批量修复 Executor 文件...\n');
