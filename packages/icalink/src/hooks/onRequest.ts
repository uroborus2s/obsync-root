...
async function authMiddleware(
  ...
  try {
    ...
    if (!verifyResult.valid || !verifyResult.payload) {
      request.log.warn(
        `无效令牌访问: ${request.url}, 错误: ${verifyResult.error}`
      );
    ...
    request.log.debug(
      `认证成功: ${verifyResult.payload.userId} - ${request.url}`
    );
  } catch (error) {
 ...
 * @param options 插件参数
 */
export async function apiOnRequest(
    ...
    async (request: FastifyRequest, reply: FastifyReply, payload: any) => {
      ...
      if (startTime) {
        ...
        if (responseTime > 1000) {
          // 超过1秒的请求记录警告
          request.log.warn(
            `慢请求: ${request.method} ${request.url} - ${responseTime}ms`
          );
        }
      }
      return payload;
