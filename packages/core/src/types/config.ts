...
export interface LifecycleHooks {
  /** 应用启动前执行，在任何插件注册完成后 fastify.listen() 之前*/
  beforeStart?: (fastify: FastifyInstance) => Promise<void> | void;
  /** 应用启动后执行，在所有插件注册完成后，fastify.listen() 之后 */
  afterStart?: (fastify: FastifyInstance) => Promise<void> | void;
  /** 应用创建fastify实例后，在任何插件注册之前 */
