...
export const isRetryableError = (error) => {
    ...
    if (error.code === 'REDIS_CONNECTION_ERROR')
        return true;
    if (error.code === 'REDIS_TIMEOUT_ERROR')
        return true;
    if (error.code === 'REDIS_NODE_UNAVAILABLE')
        return true;
    // 队列超时错误可重试
    if (error.code === 'TIMEOUT_ERROR')
        return true;
    // 消息发送错误可重试
    if (error.code === 'MESSAGE_SEND_ERROR')
        return true;
    // 网络相关错误可重试
...
export const getErrorSeverity = (error) => {
    if (!error)
        return 'low';
    // 关键错误
    if (error.code === 'REDIS_CLUSTER_ERROR')
        return 'critical';
    if (error.code === 'REDIS_OUT_OF_MEMORY')
        return 'critical';
    ...
    if (error.code === 'MESSAGE_SEND_ERROR')
        return 'medium';
    if (error.code === 'MESSAGE_RECEIVE_ERROR')
        return 'medium';
    if (error.code === 'TIMEOUT_ERROR')
        return 'medium';
    // 低级错误
