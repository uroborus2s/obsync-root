...
export class PerformanceWindow {
    ...
    getMetrics() {
        const now = Date.now();
        const recentSamples = this.samples.filter((sample) => now - sample.timestamp <= this.windowDuration);
        return {
    ...
    calculateThroughput(samples) {
        ...
        const peakMessagesPerSecond = Math.max(...Array.from(secondBuckets.values()), 0);
        return {
            messagesPerSecond,
            bytesPerSecond,
     ...
     * 计算资源使用指标
     */
    calculateResourceUsage(samples) {
...
export class PerformanceMonitor extends EventEmitter {
    ...
    constructor(config = {}) {
        super();
        this.config = config;
        this.window = new PerformanceWindow(config.maxSamples || 10000, config.windowDuration || 300000);
     ...
     * 记录操作性能
     */
    recordOperation(operation, duration, success, error, metadata) {
        const sample = {
            timestamp: Date.now(),
            operation,
            duration,
    ...
    async collect() {
        ...
        return {
            'performance.throughput.messages_per_second': metrics.throughput.messagesPerSecond,
            'performance.throughput.bytes_per_second': metrics.throughput.bytesPerSecond,
            'performance.throughput.peak_messages_per_second': metrics.throughput.peakMessagesPerSecond,
            ...
            'performance.resources.connection_count': metrics.resources.connectionCount,
 ...
 * 性能装饰器 - 用于自动记录函数执行性能
 */
export function withPerformanceMonitoring(monitor, operationName) {
