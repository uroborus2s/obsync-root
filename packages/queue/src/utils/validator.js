...
export const validateQueueConfig = (config) => {
    ...
    if (!result.isValid) {
        throw new ConfigurationError(`Invalid queue config: ${result.errors.join(', ')}`);
    }
};
// 生产者配置验证
export const validateProducerConfig = (config) => {
    const rules = [
    ...
    const result = Validator.validate(config, rules);
    if (!result.isValid) {
        throw new ConfigurationError(`Invalid producer config: ${result.errors.join(', ')}`);
    }
};
// 消费者配置验证
export const validateConsumerOptions = (options) => {
    const rules = [
    ...
    if (!result.isValid) {
        throw new ConfigurationError(`Invalid consumer options: ${result.errors.join(', ')}`);
    }
};
// 消息验证
export const validateMessage = (message) => {
    ...
    if (message.priority !== undefined) {
        if (typeof message.priority !== 'number' ||
            message.priority < 0 ||
            message.priority > 9) {
            throw new InvalidMessageError('Message priority must be a number between 0 and 9');
        }
    }
    if (message.delay !== undefined) {
        if (typeof message.delay !== 'number' || message.delay < 0) {
            throw new InvalidMessageError('Message delay must be a non-negative number');
        }
    }
    ...
    if (message.timestamp !== undefined) {
        if (typeof message.timestamp !== 'number' || message.timestamp < 0) {
            throw new InvalidMessageError('Message timestamp must be a non-negative number');
        }
    }
    if (message.retryCount !== undefined) {
        if (typeof message.retryCount !== 'number' || message.retryCount < 0) {
            throw new InvalidMessageError('Message retryCount must be a non-negative number');
        }
    }
    if (message.maxRetries !== undefined) {
        if (typeof message.maxRetries !== 'number' || message.maxRetries < 0) {
            throw new InvalidMessageError('Message maxRetries must be a non-negative number');
        }
    }
};
// 队列名称验证
export const validateQueueName = (name) => {
    ...
    if (name.length < 1 || name.length > 255) {
        throw new InvalidMessageError('Queue name must be between 1 and 255 characters');
    }
    // 队列名称只能包含字母、数字、下划线、连字符和点号
    const pattern = /^[a-zA-Z0-9_.-]+$/;
    if (!pattern.test(name)) {
        throw new InvalidMessageError('Queue name can only contain letters, numbers, underscores, hyphens, and dots');
    }
    // 不能以点号开头或结尾
...
export const validateRedisNode = (node) => {
    ...
    if (!node.port ||
        ...
        node.port < 1 ||
        node.port > 65535) {
        throw new ConfigurationError('Redis node port must be a number between 1 and 65535');
    }
};
// 批量验证
