...
export interface DeadLetterProcessOptions {
  ...
  retryOriginalQueue?: boolean; // 是否重新入队到原队列
  maxReprocessAttempts?: number; // 最大重新处理次数
}
...
export interface DeadLetterStats {
  ...
  oldestMessage?: number; // 最老消息的时间戳
  newestMessage?: number; // 最新消息的时间戳
}
...
export class DeadLetterQueueManager extends EventEmitter {
  ...
  async addMessage<T = any>(
    ...
    try {
      ...
      const redisMessageId = await connection.xadd(
        this.deadLetterKey,
        'MAXLEN',
        '~',
        this.config.maxLength || 10000,
        '*',
        ...Object.entries(messageData).flat().map(String)
      );
  ...
  ): Promise<MessageQueryResult<DeadLetterMessage<any>>> {
    try {
      const connection = this.connectionManager.getConnection();
      ...
      // 获取消息
      const messages = await connection.xrange(
        this.deadLetterKey,
        '-',
        '+',
        'COUNT',
      ...
      const parsedMessages: DeadLetterMessage<any>[] = messages
        .slice(offset)
        .map(([messageId, fields]) => {
          ...
          } as DeadLetterMessage<any>;
        });

      const total = await connection.xlen(this.deadLetterKey);

      return {
        messages: parsedMessages as Message<DeadLetterMessage<any>>[],
        total,
        offset,
        limit,
  ...
  async reprocessMessage(
    ...
    try {
      ...
      if (!originalQueue) {
        throw new Error(
          'No target queue specified and no original queue found'
        );
      }
      ...
      await connection.xadd(
        targetKey,
        '*',
        ...Object.entries(reprocessedMessage).flat().map(String)
      );
  ...
  async reprocessBatch(
    messageIds: string[],
    options: DeadLetterProcessOptions = {}
  ): Promise<{
    success: string[];
   ...
   * 解析Redis消息字段
   */
  private parseRedisMessage(fields: string[]): Record<string, string> {
  ...
  async getLength(): Promise<number> {
    try {
      ...
      this.logger.error(
        'Failed to get dead letter queue length',
        toError(error)
      );
      throw error;
    }
