...
import { createLogger, generateMessageId, Logger } from '../utils/index.js';
...
export class Queue<T = any> extends EventEmitter implements IQueue<T> {
  private connectionManager: RedisConnectionManager;
  private logger: Logger;
  private _isRunning = false;
  private streamKey: string;
  private consumerGroupKey: string;
  private deadLetterKey?: string;
  private priorityKeys: Map<number, string> = new Map();
  ...
  async send(message: Message<T>, options?: SendOptions): Promise<SendResult> {
    try {
      ...
      if (sendOptions.delay && sendOptions.delay > 0) {
        return await this.sendDelayedMessage(
          messageData,
          sendOptions.delay,
          connection
        );
      ...
      throw createQueueError.sendFailed(
        this.name,
    ...
    try {
      ...
      pipelineResults.forEach(([error, redisMessageId], index) => {
        if (error) {
          ...
          results[index].redisMessageId = redisMessageId as string;
        }
      });
  ...
  async purge(): Promise<number> {
    try {
      ...
      throw createQueueError.operationFailed(
        this.name,
        'purge',
        error instanceof Error ? error.message : 'Unknown error'
      );
      ...
      for (const priorityKey of this.priorityKeys.values()) {
        try {
          await connection.xgroup(
            'CREATE',
            priorityKey,
            `${this.consumerGroupKey}:priority`,
            '$',
            'MKSTREAM'
          );
        } catch (error) {
          ...
          if (!toError(error).message.includes('BUSYGROUP')) {
            this.logger.warn(
              `Failed to create consumer group for priority queue: ${priorityKey}`,
              error
            );
          }
        }
   ...
   * 验证消息
   */
  private validateMessage(message: Message<T>): void {
