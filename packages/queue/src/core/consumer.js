...
import { RetryPolicyFactory } from '../retry/policy.js';
import { createLogger, generateUUID, sleep } from '../utils/index.js';
...
export class Consumer extends EventEmitter {
    ...
    groupName;
    consumerName;
    constructor(queue, handler, connectionManager, options = {}) {
        super();
        ...
        if (options.deadLetterQueue) {
            ...
            this.deadLetterManager = new DeadLetterQueueManager(options.deadLetterQueue, queue.config, connectionManager);
        }
    ...
    async start() {
        try {
            ...
            this.logger.error('Failed to start consumer', error instanceof Error ? error : new Error(String(error)));
            throw error;
        }
    }
    /**
    ...
    async ensureConsumerGroup() {
        try {
            const connection = this.connectionManager.getConnection();
            await connection.xgroup('CREATE', this.streamKey, this.consumerGroup, '$', 'MKSTREAM');
        }
        catch (error) {
            // 消费者组可能已存在
    ...
    async consumeMessages() {
        try {
            ...
            // 首先处理待处理的消息
            const pendingMessages = await connection.xreadgroup('GROUP', this.consumerGroup, this.consumerId, 'COUNT', batchSize, 'STREAMS', this.streamKey, '0');
            if (pendingMessages && pendingMessages.length > 0) {
                await this.processStreamMessages(pendingMessages);
                return;
            }
            // 读取新消息
            const newMessages = await connection.xreadgroup('GROUP', this.consumerGroup, this.consumerId, 'COUNT', batchSize, 'BLOCK', timeout, 'STREAMS', this.streamKey, '>');
        ...
        for (const [_streamName, messages] of streamMessages) {
            ...
            for (const [messageId, fields] of messages) {
                try {
                    ...
                    this.logger.error(`Failed to parse message ${messageId}`, toError(error));
    ...
    async handleMessageError(messageId, error) {
        try {
            ...
            this.logger.error(`Failed to ack error message ${messageId}`, toError(ackError));
        }
    }
    /**
     * 处理处理错误
     */
    async handleProcessingError(results, error) {
        ...
        for (const result of results) {
            try {
                ...
                this.logger.error(`Failed to handle processing error for message ${result.messageId}`, toError(handleError));
            }
        }
    }
    /**
     * 重试消息
     */
    async retryMessage(result, retryCount, lastError) {
        try {
            // 使用重试策略计算延迟
            const delay = this.retryPolicy.calculateDelay(retryCount, lastError);
            if (delay === -1) {
                this.logger.warn(`Retry policy rejected retry for message ${result.messageId}`);
                // 如果重试策略拒绝重试，发送到死信队列
                await this.sendToDeadLetter(result);
                await result.ack();
                return;
            }
            this.logger.debug(`Retrying message ${result.messageId} after ${delay}ms delay`, {
                retryCount,
                delay,
            ...
            this.logger.error(`Failed to retry message ${result.messageId}`, toError(error));
            throw error;
        }
    }
    /**
     * 发送到死信队列
     */
    async sendToDeadLetter(result) {
        if (!this.deadLetterManager) {
            this.logger.warn(`No dead letter queue configured for message ${result.messageId}`);
            return;
        }
        try {
            await this.deadLetterManager.addMessage(result.message, 'max_retries_exceeded', this.queue.name, {
            ...
            this.logger.error(`Failed to send message ${result.messageId} to dead letter queue`, toError(error));
            throw error;
        }
    }
    /**
    ...
    async waitForProcessingComplete() {
        ...
        this.logger.info(`Waiting for ${this.processingPromises.size} processing tasks to complete...`);
