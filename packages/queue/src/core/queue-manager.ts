...
export class QueueManager extends EventEmitter implements IQueueManager {
  ...
  constructor(public readonly config: QueueManagerConfig) {
    super();
   ...
   * 创建队列
   */
  async createQueue(name: string, config?: QueueConfig): Promise<IQueue> {
    try {
      // 验证队列名称
  ...
  async getMetrics(): Promise<Metrics> {
    try {
      // 收集队列指标
      const queueMetricsArray = await Promise.all(
        Array.from(this.queues.values()).map(async (queue) => {
          try {
      ...
      queueMetricsArray
        .filter((item): item is { name: string; stats: any } => item !== null)
        .forEach(({ name, stats }) => {
          queues[name] = stats;
        });
   ...
   * 验证配置
   */
  private validateConfig(config: QueueManagerConfig): void {
    if (!config.redis) {
    ...
    if (!config.redis.cluster && !config.redis.single) {
      throw new QueueConfigurationError(
        'Either cluster or single Redis configuration must be provided'
      );
    }
  }
  ...
  private validateQueueName(name: string): void {
    if (!name || typeof name !== 'string') {
      throw createQueueError.invalidName(
   ...
   * 设置队列事件监听
   */
  private setupQueueEvents(queue: IQueue): void {
   ...
   * 启动所有队列
   */
  private async startAllQueues(): Promise<void> {
    const startPromises = Array.from(this.queues.values()).map((queue) =>
      queue.start()
    );
   ...
   * 停止所有队列
   */
  private async stopAllQueues(): Promise<void> {
    const stopPromises = Array.from(this.queues.values()).map((queue) =>
      queue.stop()
    );
