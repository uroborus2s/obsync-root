...
import { createLogger, Logger, sleep } from '../utils/index.js';
...
export class Producer<T = any> extends EventEmitter implements IProducer<T> {
   ...
   * 判断是否应该批处理
   */
  private shouldBatch(message: Message<T>, options?: SendOptions): boolean {
    // 高优先级消息不批处理
    ...
    for (const item of retryItems) {
      try {
        ...
        if (item.attempt >= (this.config.maxRetries || 3)) {
          item.reject(
            createQueueError.maxRetriesExceeded(this.queue.name, item.attempt)
          );
        } else {
          // 重新加入重试队列
          this.retryQueue.push({
  ...
  private updateMetrics(
    ...
    // 计算错误率（简化实现）
    const totalAttempts =
      this.metrics.messagesSent + (success ? 0 : messageCount);
