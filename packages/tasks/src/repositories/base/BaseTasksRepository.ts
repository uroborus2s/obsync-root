...
import { BaseRepository, QueryError } from '@stratix/database';
import {
  eitherLeft,
  eitherRight,
  maybeNone,
...
type LegacyDatabaseResult<T> = DatabaseResult<T>;
...
export const toOption = <T>(value: T | null | undefined): Option<T> => {
  return value != null ? maybeSome(value) : maybeNone;
};
...
> extends BaseRepository<TasksDatabase, TB, T, CreateT, UpdateT> {
  ...
  protected toLegacySuccess<TValue>(data: TValue): LegacyDatabaseResult<TValue> {
    return eitherRight(data);
  }

  protected toLegacyFailure(error: unknown): LegacyDatabaseResult<never> {
    return eitherLeft(
      error instanceof QueryError
        ? error
        : QueryError.create(error instanceof Error ? error.message : String(error))
    );
  }

  async findById(id: string | number): Promise<any> {
    return this.toLegacySuccess(await super.findById(id));
  }

  async findOne(criteria: any): Promise<any> {
    return this.toLegacySuccess(await super.findOne(criteria));
  }

  async findMany(criteria?: any, options?: any): Promise<any> {
    return this.toLegacySuccess(await super.findMany(criteria, options));
  }

  async count(criteria?: any): Promise<any> {
    return this.toLegacySuccess(await super.count(criteria));
  }
  ...
  async delete(id: string | number): Promise<any> {
    ...
    return this.toLegacySuccess(true);
  }

  /**
   * 便捷方法：findById 返回 T | null 类型
   */
  async findByIdNullable(id: IdType): Promise<LegacyDatabaseResult<T | null>> {
    const result = await this.findById(id as string | number);
    if (result.success) {
      return this.toLegacySuccess(fromOption(result.data));
    }
    return this.toLegacyFailure(result.error);
  }
  ...
  async updateNullable(
    id: IdType,
    data: UpdateT
  ): Promise<LegacyDatabaseResult<T | null>> {
