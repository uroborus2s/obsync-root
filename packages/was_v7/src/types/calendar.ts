...
/**
 * 日程时间
 * date 与 datetime 互斥，二选一，非全天场景下使用此字段
 */
export interface ScheduleDateTime {
  /** 时间，格式需满足 RFC3339(如“2024-01-15T14:30:00+08:00”) */
  datetime?: string;
...
export interface UpdateScheduleBodyParams {
  ...
  /** 是否重置参与者邀请状态（修改时间/重复规则时生效），默认 true */
  is_reinvition?: boolean;
  /** 地址数组，最多 1 个对象 */
  locations?: Array<{ name: string }>;
  /** 针对重复日程的编辑类型，可选值：one（修改单个实例） / all（修改全部实例） */
  mod_type?: 'one' | 'all';
  /** 在线会议对象，null 表示清除 */
  online_meeting?: OnlineMeeting | null;
  /** 重复规则对象 */
  recurrence?: UpdateRecurrenceRule;
  /** 提醒数组，最多 10 个对象 */
  reminders?: ScheduleReminder[];
  /** 日程标题 */
  summary?: string;
  ...
  visibility?: 'default' | 'public' | 'private';
  /** 针对重复日程 mod_type=one 时必填，用于指定要修改的实例开始时间（毫秒时间戳） */
  which_day_time?: number;
}

/**
 * 更新日程时使用的重复规则（不包含 exdate，因为 exdate 是只读字段）
 */
export interface UpdateRecurrenceRule {
  /** 重复频率：YEARLY | MONTHLY | WEEKLY | DAILY | LUNAR_YEARLY */
  freq: 'YEARLY' | 'MONTHLY' | 'WEEKLY' | 'DAILY' | 'LUNAR_YEARLY';
  /** 重复间隔，例如 2 表示每两周一次 */
  interval?: number;
...
export interface UpdateScheduleParams {
  /** 路径参数：当前日历 ID，可使用 'primary' 表示主日历 */
  calendar_id: string;
  /** 路径参数：日程 ID */
  event_id: string;
  /** 请求体参数 */
  body?: UpdateScheduleBodyParams;
}
