...
describe('WPS V7 所有适配器测试', () => {
  ...
  describe('日程适配器', () => {
    it('应该包含6个核心日程管理方法', () => {
      const adapter = createWpsScheduleAdapter(container);

      expect(typeof adapter.createSchedule).toBe('function');
      expect(typeof adapter.batchCreateSchedules).toBe('function');
      expect(typeof adapter.deleteSchedule).toBe('function');
      expect(typeof adapter.getScheduleList).toBe('function');
      expect(typeof adapter.getSchedule).toBe('function');

      // 确保只有这5个方法（简化版本）
      const methods = Object.getOwnPropertyNames(adapter);
  ...
  describe('适配器配置', () => {
    it('每个适配器都应该有正确的默认导出配置', () => {
      const expectedConfigs = [
        { name: 'user', factory: createWpsUserAdapter },
        { name: 'department', factory: createWpsDepartmentAdapter },
        { name: 'company', factory: createWpsCompanyAdapter },
        { name: 'calendar', factory: createWpsCalendarAdapter },
        { name: 'schedule', factory: createWpsScheduleAdapter },
        { name: 'message', factory: createWpsMessageAdapter },
        { name: 'chat', factory: createWpsChatAdapter },
        { name: 'userAuth', factory: createWpsUserAuthAdapter }
      ];
  ...
  describe('认证流程', () => {
    it('所有适配器都应该在API调用前检查token有效性', async () => {
      const userAdapter = createWpsUserAdapter(container);
    ...
    it('应该在token过期时自动刷新', async () => {
      ...
      await messageAdapter.sendTextMessage(
        [{ receiver_type: 'user', receiver_id: 'user123' }],
        '测试消息'
      );
