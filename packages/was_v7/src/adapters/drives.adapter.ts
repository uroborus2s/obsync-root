...
export interface WpsDriveAdapter {
  ...
  // 异步任务管理
  getDriveTaskMeta(params: GetDriveTaskMetaParams): Promise<DriveTaskMeta>;
}
...
export function createWpsDriveAdapter(
  ...
  const adapter: WpsDriveAdapter = {
    ...
    async deleteFile(params: DeleteFileParams): Promise<void> {
      await httpClient.ensureAccessToken();
      const { file_id, drive_id } = params;

      await httpClient.post(`/v7/drives/${drive_id}/files/${file_id}/delete`);
    },

    /**
     * 批量删除文件(夹)
     * 批量删除指定的文件或文件夹，返回异步任务ID
     *
     * @param params - 批量删除参数
     * @returns 包含异步任务ID的响应对象
     *
     * @example
     * ```typescript
     * const result = await adapter.batchDeleteFiles({
     *   drive_id: 'q60YOE5',
    ...
    async batchDeleteFiles(
      ...
      await httpClient.ensureAccessToken();
      const { drive_id, file_ids } = params;

      const response = await httpClient.post<BatchDeleteFilesResponse>(
        `/v7/drives/${drive_id}/files/batch_delete`,
        { file_ids }
      );
    ...
    async renameFile(params: RenameFileParams): Promise<RenameFileResponse> {
      await httpClient.ensureAccessToken();
      const { file_id, name, drive_id } = params;

      const response = await httpClient.post<RenameFileResponse>(
        `/v7/drives/${drive_id}/files/${file_id}/rename`,
        { name }
      );
      return response.data;
    },
    ...
    /**
     * 获取异步任务信息
     * 查询异步任务执行进度及结果（成功/失败数量、状态等）
     *
     * @param params - 任务查询参数
     * @returns 任务执行信息，包含完成数量、失败数量、状态和总数
     *
     * @example
     * ```typescript
     * // 查询批量删除任务的执行状态
     * const taskInfo = await adapter.getDriveTaskMeta({
     ...
     * ```
     */
    async getDriveTaskMeta(
      params: GetDriveTaskMetaParams
    ): Promise<DriveTaskMeta> {
      await httpClient.ensureAccessToken();
      const { task_id } = params;

      const response = await httpClient.get<DriveTaskMeta>(
        `/v7/drive_tasks/${task_id}/meta`
      );
