...
| 启动集成             | `application-discovery-bootstrap.test.ts`          | `Stratix.run()` 到 DI、请求 scope、路由响应、response schema failure envelope 和 production manifest consumption                                              |
...
| `CI=true pnpm --filter @stratix/core exec vitest run`               | 通过，27 files / 191 tests |
...
| `RISK-CORE-013` | production manifest 配置漂移或失效            | bootstrap/config 测试必须覆盖 `discovery.productionManifest` 验收、strict invalid manifest fail-fast、loaded artifact 暴露和 `skipRuntimeDiscovery` 跳过 runtime glob discovery |
