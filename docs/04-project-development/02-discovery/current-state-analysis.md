...
| `CI=true pnpm --filter @stratix/core exec vitest run` | passed | P1 RegistrationPlan 收敛后，`29` 个测试文件、`213` 个测试全部通过；覆盖 route contract/OpenAPI、统一错误 envelope、response schema failure 归一化、plan-first DI diagnostics、plugin adapter RegistrationPlan metadata、discovery schema validation、DI graph、production manifest startup consumption、manifest-driven registration、observability/security preset 和 ApplicationDiscoveryRegistrar |
...
| `pnpm --filter @stratix/forge test`                   | passed | `48` 个测试全部通过；覆盖 `doctor di`、`di graph`、`doctor modules`、`graph modules`、`doctor plugins`、`graph plugins`、`build-manifest`、project/workspace release gate、release pack artifact gate、release metadata gate、registry exact-version gate、`openapi generate`、高级 `openapi client`、命令 help 和生成资源 DI/schema 检查               |
| `pnpm --filter @stratix/core run test:coverage`       | passed | core coverage ratchet 通过：global lines `44.00%`、functions `38.49%`、branches `34.62%`、statements `43.19%`；bootstrap/discovery 关键路径使用更高目标阈值                                                                                                                                                                    |
