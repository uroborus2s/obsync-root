...
| `pnpm lint`                                                      | passed | 根入口委托 `lint:supported`，`11/11` 个 supported lint tasks 通过；包含 DevTools client，排除 `@stratix/tasks`                                                                                                                                                                                                |
...
| `CI=true pnpm --filter @stratix/core exec vitest run` | passed | P2+ provider 与健康检查硬化加入后，`31` 个测试文件、`224` 个测试全部通过；覆盖 route contract/OpenAPI、统一错误 envelope、response schema failure 归一化、plan-first DI diagnostics、plugin adapter RegistrationPlan metadata、discovery schema validation、DI graph、production manifest startup consumption、manifest-driven registration、production manifest v2 artifact integrity / compiled-file registration、observability/security provider、readiness/liveness、runtime stability 和 ApplicationDiscoveryRegistrar |
...
| `pnpm --filter @stratix/forge test`                   | passed | `51` 个测试全部通过；覆盖 `doctor di`、`di graph`、`doctor modules`、`graph modules`、adapter-backed `doctor plugins`、`graph plugins`、production manifest v2 `build-manifest`、artifact hash release gate、project/workspace release gate、release pack artifact gate、release metadata gate、registry exact-version gate、`openapi generate`、高级 `openapi client`、命令 help 和生成资源 DI/schema 检查               |
| `pnpm --filter @stratix/core run test:coverage`       | passed | core coverage ratchet 通过：global lines `45.09%`、functions `39.27%`、branches `35.69%`、statements `44.27%`；bootstrap/discovery 关键路径使用更高目标阈值；P2+ 后仍高于 global ratchet lines `43`、functions `38`、branches `34`、statements `42`                                                                                                                                                                    |
| `pnpm run quality:release`                            | passed | 2026-06-19 本地完整通过：10 个 supported package build、显式 supported typecheck、12 个 supported test turbo tasks、core coverage ratchet、86 pages / 0 contracts docs 校验、production 高危安全审计、workspace release gate dry-run |
...
| `uvx --from docs-stratego docs-stratego source validate --repo-path .`                                                                                                                                                                                                                                                                | passed                   | 86 pages / 0 contracts                                                                                       |
...
- `@stratix/core` 已提供 route contract extraction、contract diagnostics、OpenAPI document generation、DI graph 和 DI diagnostics 的基础 API；应用 discovery 和插件 AutoDI 可生成统一 `RegistrationPlan`，应用 DI token 与插件 adapter root token 通过 plan token registrar 注册和记录，DI graph / missing dependency diagnostics 携带 plan metadata。
...
建议把软件工厂当前阶段定为 `PHASE_6_LOCAL_95_CONTROLLED_RELEASE`。
...
- P2+ 已补齐 provider 插拔化和插件 manifest `provides` 深校验，本地生产级评分达到 `95/100`
