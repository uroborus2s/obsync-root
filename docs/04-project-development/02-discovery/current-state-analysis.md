...
| `CI=true pnpm --filter @stratix/core exec vitest run` | passed | P2 production manifest v2 与运行稳定性测试加入后，`31` 个测试文件、`221` 个测试全部通过；覆盖 route contract/OpenAPI、统一错误 envelope、response schema failure 归一化、plan-first DI diagnostics、plugin adapter RegistrationPlan metadata、discovery schema validation、DI graph、production manifest startup consumption、manifest-driven registration、production manifest v2 artifact integrity / compiled-file registration、observability/security preset、runtime stability 和 ApplicationDiscoveryRegistrar |
...
| `pnpm --filter @stratix/forge test`                   | passed | `49` 个测试全部通过；覆盖 `doctor di`、`di graph`、`doctor modules`、`graph modules`、`doctor plugins`、`graph plugins`、production manifest v2 `build-manifest`、artifact hash release gate、project/workspace release gate、release pack artifact gate、release metadata gate、registry exact-version gate、`openapi generate`、高级 `openapi client`、命令 help 和生成资源 DI/schema 检查               |
| `pnpm --filter @stratix/core run test:coverage`       | passed | core coverage ratchet 通过：global lines `44.63%`、functions `39.12%`、branches `35.27%`、statements `43.83%`；bootstrap/discovery 关键路径使用更高目标阈值；P2 将 global ratchet 上调为 lines `43`、functions `38`、branches `34`、statements `42`                                                                                                                                                                    |
...
- `@stratix/core` 已提供 Phase 5/P2 production baseline：`discovery.productionManifest` 可在启动期读取并校验 `.stratix/production-manifest.json`，把 artifact 暴露到应用实例，在 `skipRuntimeDiscovery: true` 时跳过应用级 runtime glob discovery，并在 `registerFromManifest: true` 时优先按 v2 manifest `compiledFile` 注册 DI 和路由，v1 manifest 继续按 source files 兼容注册；`config.observability` 提供 request/trace id、health、metrics、traces；`config.security` 提供 body limit、CORS、headers 和 rate limit。
