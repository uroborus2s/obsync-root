...
- 通过 `src/adapters`、`src/services`、`src/repositories`、`src/controllers` 这些目录暴露能力
...
如果你已经安装了 create，可以直接运行：

```bash
create-stratix plugin integration @acme/ping-plugin
```

如果你是在本仓库里验证本地 create，可以运行：

```bash
node packages/create/dist/bin/create-stratix.js plugin integration @acme/ping-plugin
```

如果你想让依赖先不自动安装，可以加 `--no-install`：

```bash
create-stratix plugin integration @acme/ping-plugin --pm pnpm --no-install
```
