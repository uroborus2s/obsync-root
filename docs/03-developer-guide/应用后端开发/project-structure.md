...
它通常应该由 CI 或发布流程生成，不建议手工编辑。生产配置可通过 `discovery.productionManifest` 读取它；当 `skipRuntimeDiscovery: true` 时，启动会跳过应用级 runtime glob discovery。当前 runtime 基线只消费 manifest 作为启动证据，不从 manifest 反向恢复 DI/路由注册。
