...
# 日志函数
log() { echo -e "\033[0;33m[WEB]\033[0m $1"; }
log_success() { echo -e "\033[0;32m✅ [WEB]\033[0m $1"; }
...
check_source_directory() {
    log "检查 agendaedu-web 源码目录..."
    
    if [ ! -d "$WEB_SOURCE_DIR" ]; then
        log_error "源码目录不存在: $WEB_SOURCE_DIR"
        return 1
    fi
    
    if [ ! -f "$WEB_SOURCE_DIR/package.json" ]; then
        log_error "package.json 不存在: $WEB_SOURCE_DIR/package.json"
        return 1
    fi
...
build_static_files() {
    ...
    # 检查构建结果
    if [ ! -d "$WEB_BUILD_DIR" ]; then
        log_error "构建目录不存在: $WEB_BUILD_DIR"
        return 1
    fi
    
    local file_count=$(find "$WEB_BUILD_DIR" -type f | wc -l)
    local dir_size=$(du -sh "$WEB_BUILD_DIR" | cut -f1)
...
backup_existing_files() {
    ...
    # 备份现有文件
    remote_exec "if [ -d $STATIC_WEB_DIR ]; then tar -czf $BACKUP_DIR/static/agendaedu-web.backup.$DEPLOY_TIMESTAMP.tar.gz -C $STATIC_WEB_DIR . 2>/dev/null || true; fi" "备份现有 Web 静态文件"
...
# 部署静态文件
deploy_static_files() {
    log "部署 agendaedu-web 静态文件到 $SERVER_NAME..."
    
    if [ "$DEPLOY_DRY_RUN" = "true" ]; then
        log "[DRY-RUN] 部署静态文件: $WEB_BUILD_DIR -> $STATIC_WEB_DIR"
        return 0
    fi
    ...
    log "同步静态文件..."
    local temp_dir="/tmp/static-web-$$"

    if rsync -avz --delete "$WEB_BUILD_DIR/" "$SERVER_HOST:$temp_dir/"; then
        # 移动到目标目录
        if remote_exec "sudo rm -rf $STATIC_WEB_DIR/* && sudo mv $temp_dir/* $STATIC_WEB_DIR/" "移动静态文件到目标目录"; then
            # 移动成功后清理临时目录
    ...
    # 设置文件权限
    remote_exec "sudo chown -R www-data:www-data $STATIC_WEB_DIR" "设置文件所有者"
    remote_exec "sudo find $STATIC_WEB_DIR -type f -exec chmod 644 {} \;" "设置文件权限"
    remote_exec "sudo find $STATIC_WEB_DIR -type d -exec chmod 755 {} \;" "设置目录权限"
...
verify_deployment() {
    ...
    # 检查目录存在
    if ! remote_exec "test -d $STATIC_WEB_DIR" "检查静态文件目录"; then
        log_error "静态文件目录不存在"
        return 1
    fi
    
    # 检查关键文件
    if ! remote_exec "test -f $STATIC_WEB_DIR/index.html" "检查 index.html"; then
        log_error "index.html 文件不存在"
        return 1
    fi
    
    # 统计文件数量
    local remote_file_count=$(ssh $SSH_OPTIONS "$SERVER_HOST" "find $STATIC_WEB_DIR -type f | wc -l")
    log "远程文件数量: $remote_file_count"
    
    # 测试 HTTP 访问
    if remote_exec "curl -s -f http://localhost/web/ >/dev/null" "测试 Web 访问"; then
        log_success "Web 静态文件访问正常"
    else
...
generate_deployment_report() {
    ...
    local report_file="$SCRIPT_DIR/deploy-web-report-$DEPLOY_TIMESTAMP.txt"
...
- 构建命令: $WEB_BUILD_COMMAND

部署结果:
- 状态: 成功
- 文件数量: $(find "$WEB_BUILD_DIR" -type f | wc -l)
- 总大小: $(du -sh "$WEB_BUILD_DIR" | cut -f1)
...
# 主函数
main() {
    log "=== 开始部署 agendaedu-web 静态文件 ==="
    
    # 获取服务器信息
    get_server_info
    
    log "目标服务器: $SERVER_NAME ($SERVER_HOST)"
    log "源码目录: $WEB_SOURCE_DIR"
    log "构建目录: $WEB_BUILD_DIR"
    ...
    log_success "=== agendaedu-web 静态文件部署完成 ==="
}
